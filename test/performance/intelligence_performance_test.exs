defmodule EveDmv.Performance.IntelligencePerformanceTest do
  ...
  @max_character_analysis_time 500  # milliseconds
    ...
    test "performs complete vetting analysis within time limit" do
      ...
      {time_microseconds, _results} = :timer.tc(fn ->
        %{
          ...
          security: WHVettingAnalyzer.analyze_security_risks(character_data, employment_history),
  ...
  defp build_fleet_killmail_data(corp_id, fleet_members) do
    %{
      "attackers" => Enum.map(fleet_members, fn member_id ->
        %{
  ...
  defp generate_vetting_killmails(count) do
    for i <- 1..count do
      %{
        ...
        attacker_corporation_name: if(rem(i, 50) == 0, do: "Hard Knocks Citizens", else: "Corp #{i}"),

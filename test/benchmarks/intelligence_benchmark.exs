defmodule EveDmv.IntelligenceBenchmark do
  ...
  alias EveDmv.Intelligence.{CharacterAnalyzer, HomeDefenseAnalyzer, WHFleetAnalyzer, WHVettingAnalyzer}
  ...
  defp benchmark_home_defense_analyzer do
    {:ok, _analysis} = HomeDefenseAnalyzer.analyze_home_defense(
      @test_corporation_id, 
      @test_home_system_id
    )
  end
  ...
  defp benchmark_wh_vetting_analyzer do
    ...
    %{
      ...
      security_risks: WHVettingAnalyzer.analyze_security_risks(character_data, employment_history),
  ...
  defp benchmark_batch_analysis do
    ...
    tasks = for character_id <- @test_characters do
      Task.async(fn ->
  ...
  defp create_character_activity(character_id, kill_count) do
    for i <- 1..kill_count do
      killmail_data = %{
        ...
        "attackers" => [
          %{
            "character_id" => if(rem(i, 3) == 0, do: Enum.random(90_000_000..95_000_000), else: character_id),
        ...
        "victim" => %{
          "character_id" => if(rem(i, 3) == 0, do: character_id, else: Enum.random(90_000_000..95_000_000)),
          "corporation_id" => Enum.random(1_000_000..2_000_000),
          "ship_type_id" => Enum.random([587, 588, 589, 17738, 29984]),
  ...
  defp generate_killmail_history(count) do
    for i <- 1..count do
      %{
        ...
        solar_system_id: if(rem(i, 2) == 0, do: Enum.random(31_000_000..31_005_000), else: Enum.random(30_000_000..30_005_000)),
        is_victim: rem(i, 4) == 0,
        attacker_count: Enum.random(1..10),
        attacker_character_name: "Pilot #{i}",
        attacker_corporation_name: if(rem(i, 20) == 0, do: "Hard Knocks Citizens", else: "Corp #{i}"),
        attacker_alliance_name: if(rem(i, 20) == 0, do: "Hard Knocks Citizens", else: "Alliance #{i}"),

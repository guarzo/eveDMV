defmodule EveDmv.Surveillance.MatchingEngine do
  ...
  def handle_call({:match_killmail, killmail}, _from, state) do
    ...
    try do
      ...
      :telemetry.execute([:eve_dmv, :surveillance, :profile, :evaluated], %{count: length(candidates)}, %{})
      
      if length(matches) > 0 do
        :telemetry.execute([:eve_dmv, :surveillance, :profile, :match], %{count: length(matches)}, %{})
      end
      
      Logger.debug("Matched killmail #{killmail["killmail_id"]} against #{length(candidates)} candidates in #{duration}μs, #{length(matches)} matches")
  ...
  defp compile_rule(%{"field" => field, "operator" => operator, "value" => value}) do
    compiled_fn = case {field, operator} do
      # Numeric comparisons
      {field, "eq"} -> fn km -> get_field(km, field) == value end
      {field, "ne"} -> fn km -> get_field(km, field) != value end
      {field, "gt"} -> fn km -> compare_numeric(get_field(km, field), value, :gt) end
      {field, "lt"} -> fn km -> compare_numeric(get_field(km, field), value, :lt) end
      {field, "gte"} -> fn km -> compare_numeric(get_field(km, field), value, :gte) end
      {field, "lte"} -> fn km -> compare_numeric(get_field(km, field), value, :lte) end
      
      # List operations
      {field, "in"} -> fn km -> get_field(km, field) in value end
      {field, "not_in"} -> fn km -> get_field(km, field) not in value end
      
      # Array field operations
      {field, "contains_any"} -> fn km -> 
        field_value = get_field(km, field) || []
        is_list(field_value) and not MapSet.disjoint?(MapSet.new(field_value), MapSet.new(value))
      end
      {field, "contains_all"} -> fn km ->
        field_value = get_field(km, field) || []
        is_list(field_value) and MapSet.subset?(MapSet.new(value), MapSet.new(field_value))
      end
      {field, "not_contains"} -> fn km ->
        field_value = get_field(km, field) || []
  ...
  defp get_field(killmail, field) do
    case field do
      ...
      "ship_value" -> killmail["ship_value"] || get_in(killmail, ["zkb", "destroyedValue"]) || 0
      "fitted_value" -> killmail["fitted_value"] || get_in(killmail, ["zkb", "fittedValue"]) || 0
      ...
      "kill_category" -> classify_kill(killmail)
      "victim_ship_category" -> classify_ship(get_in(killmail, ["victim", "ship_type_id"]))
      "module_tags" -> killmail["module_tags"] || []
      "noteworthy_modules" -> killmail["noteworthy_modules"] || []
      "victim_character_name" -> get_in(killmail, ["victim", "character_name"])
      "victim_corporation_name" -> get_in(killmail, ["victim", "corporation_name"])
      "victim_alliance_name" -> get_in(killmail, ["victim", "alliance_name"])
  ...
  defp build_indexes_for_profile(profile) do
    ...
    case extract_indexable_fields(profile.filter_tree) do
      {:ok, fields} ->
        Enum.each(fields, fn {field, values} ->
          case field do
            "module_tags" -> 
              Enum.each(values, fn tag -> 
                :ets.insert(@index_by_tag, {tag, profile.id})
              end)
            "solar_system_id" ->
              Enum.each(values, fn sys_id ->
                :ets.insert(@index_by_system, {sys_id, profile.id})
              end)
            "victim_ship_type_id" ->
              Enum.each(values, fn ship_id ->
                :ets.insert(@index_by_ship, {ship_id, profile.id})
              end)
            "total_value" ->
              # For ISK thresholds, store ranges
              Enum.each(values, fn {operator, value} ->
                :ets.insert(@index_by_isk, {{operator, value}, profile.id})
              end)
            _ -> :ok
          end
        end)

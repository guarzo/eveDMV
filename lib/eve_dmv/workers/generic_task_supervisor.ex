defmodule EveDmv.Workers.GenericTaskSupervisor do
  ...
  defmacro __using__(_opts) do
    quote do
      ...
      def start_task(task_fn, options \\ [], metadata \\ %{}) do
        EveDmv.Workers.GenericTaskSupervisor.TaskRunner.start_task(
          __MODULE__,
          @table_name,
          task_fn,
          options,
      ...
      """
      def get_stats do
        EveDmv.Workers.GenericTaskSupervisor.TaskStats.get_stats(__MODULE__, @table_name)
      end
      ...
      def get_running_tasks do
        EveDmv.Workers.GenericTaskSupervisor.TaskStats.get_running_tasks(@table_name)
      end
    end
  end
  ...
  defmodule TaskRunner do
    ...
    defp start_monitoring(pid, config, table_name, _metadata) do
      spawn(fn ->
        EveDmv.Workers.GenericTaskSupervisor.TaskMonitor.monitor_task(pid, config, table_name)
      end)
    end
  ...
  defmodule TaskMonitor do
    ...
    defp handle_warning(pid, _config, _table_name, task_info) do

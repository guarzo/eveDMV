defmodule EveDmv.Database.QueryPlanAnalyzer.TableStatsAnalyzer do
      ...
      case stats_rows do
        [
          ...
          %{
            ...
            usage_patterns: analyze_usage_patterns(seq_scan, idx_scan, seq_tup_read, idx_tup_fetch),
            recommendations:
              generate_table_recommendations(seq_scan, idx_scan, n_dead_tup, n_live_tup, last_analyze)
          }
  ...
  def identify_maintenance_candidates(table_stats) do
    candidates = Enum.filter(table_stats, fn table ->
  ...
  def generate_maintenance_commands(table_name, issues) do
    ...
    commands =
      if "high_bloat" in issues do
        ["VACUUM FULL #{table_name};" | commands]
      else
        commands
      end
    ...
    commands =
      if "missing_indexes" in issues do
        ["-- Review and add appropriate indexes for #{table_name}" | commands]
      else
        commands
      end
    ...
    commands =
      if "high_sequential_scans" in issues do
        ["-- Consider adding indexes to reduce sequential scans on #{table_name}" | commands]
      else
        commands
      end
  ...
  defp analyze_usage_patterns(seq_scan, idx_scan, seq_tup_read, idx_tup_fetch) do
    ...
    %{
      ...
      avg_tuples_per_seq_scan: if(seq_scan && seq_scan > 0, do: (seq_tup_read || 0) / seq_scan, else: 0),
      avg_tuples_per_idx_scan: if(idx_scan && idx_scan > 0, do: (idx_tup_fetch || 0) / idx_scan, else: 0),
  ...
  defp classify_scan_efficiency(seq_scan, idx_scan, _seq_tup_read, _idx_tup_fetch) do
    seq_ratio = if (seq_scan || 0) + (idx_scan || 0) > 0, do: (seq_scan || 0) / ((seq_scan || 0) + (idx_scan || 0)), else: 0
  ...
  defp generate_table_recommendations(seq_scan, idx_scan, dead_tuples, live_tuples, last_analyze) do
    ...
    recommendations =
      if stale_statistics?(%{last_analyze: last_analyze}) do
        ["Table statistics are stale - run ANALYZE for better query planning" | recommendations]
      else
        recommendations
      end
  ...
  defp stale_statistics?(maintenance_info) do
    ...
    case last_analyze do
      ...
      %DateTime{} = datetime -> DateTime.diff(DateTime.utc_now(), datetime, :day) > 7
      _ -> true
    end
  end

  defp calculate_overall_health(table_stats) do
    if length(table_stats) == 0 do
  ...
  defp calculate_table_health_score(table) do
    bloat_score = max(0, 100 - (table.bloat_ratio * 500))  # Penalty for bloat
    scan_score = table.usage_patterns.index_scan_ratio * 100  # Reward for index usage
  ...
  defp analyze_bloat_patterns(table_stats) do
    ...
    %{
      ...
      avg_bloat_ratio: if(length(table_stats) > 0, do: Enum.sum(Enum.map(table_stats, & &1.bloat_ratio)) / length(table_stats), else: 0),
      highest_bloat: if(length(table_stats) > 0, do: Enum.max_by(table_stats, & &1.bloat_ratio), else: nil)
    }
  end
    ...
    %{
      avg_index_usage: 
        if(length(table_stats) > 0, 
          do: Enum.sum(Enum.map(table_stats, & &1.usage_patterns.index_scan_ratio)) / length(table_stats), 
          else: 0),
      tables_with_poor_efficiency: Enum.count(table_stats, & &1.usage_patterns.sequential_scan_ratio > 0.5)
    }
  end
  ...
  defp analyze_maintenance_status(table_stats) do
    ...
    %{
      ...
      maintenance_urgency: classify_maintenance_urgency(needs_vacuum, needs_analyze, length(table_stats))
    }
  end
  ...
  defp calculate_maintenance_priority(table) do
    ...
    priority = priority + if table.live_tuples > 1_000_000, do: 15, else: 0  # Large table bonus
  ...
  defp identify_table_issues(table) do
    ...
    issues = if stale_statistics?(table.maintenance_info), do: ["stale_statistics" | issues], else: issues
    issues = if table.usage_patterns.sequential_scan_ratio > 0.3, do: ["high_sequential_scans" | issues], else: issues
    issues = if length(table.column_stats) == 0, do: ["missing_column_stats" | issues], else: issues
  ...
  defp get_maintenance_actions(table) do
    ...
    actions = if table.bloat_ratio > 0.2, do: ["VACUUM #{table.table_name}" | actions], else: actions
    actions = if stale_statistics?(table.maintenance_info), do: ["ANALYZE #{table.table_name}" | actions], else: actions
    actions = if table.usage_patterns.sequential_scan_ratio > 0.3, do: ["Review indexing strategy" | actions], else: actions
  ...
  defp generate_index_recommendations_for_table(table) do
    ...
    recommendations =
      if table.usage_patterns.avg_tuples_per_seq_scan > 10000 do
  ...
  defp estimate_daily_growth(table) do
    ...
    inserts_per_day = (table.tuples_inserted || 0) / 7  # Rough weekly average
    inserts_per_day
  end

  defp classify_growth_trend(daily_growth) do
    cond do
      daily_growth > 10000 -> "High Growth"

defmodule EveDmv.Users.TokenRefreshService do
      ...
      {:ok, users} ->
        if Enum.empty?(users) do
          ...
          refresh_results =
            ...
            |> Enum.reduce({0, 0}, &count_refresh_result/2)
    ...
    user =
      case user_or_user_id do
        ...
        user_id when is_binary(user_id) ->
          case Ash.get(User, user_id, domain: EveDmv.Api) do
            {:ok, user} -> user
            {:error, _} -> nil
          end
        ...
        {:ok, %{status: 200, body: response_body}} ->
          case Jason.decode(response_body) do
            {:ok, token_data} ->
              ...
              expires_at = calculate_token_expiration(token_data)
  ...
  defp count_refresh_result(result, {success_count, error_count}) do
    case result do

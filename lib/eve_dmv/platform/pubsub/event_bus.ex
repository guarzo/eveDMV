defmodule EveDmv.Platform.PubSub.EventBus do
  @moduledoc """
  Centralized event bus for domain event publishing and subscription.
  ...
  ## Usage
      ...
      # Subscribe a process
      EventBus.subscribe_process(:killmail_processed)

  ## Event Types

  - `:battle_detected` - New battle identified
  - `:killmail_processed` - Killmail processing completed
  - `:character_analyzed` - Character analysis completed
  ...
  def broadcast(topic, message) do
    Logger.debug("Broadcasting message to topic", %{
      topic: topic,
    ...
    case PubSub.broadcast(@pubsub, topic, message) do
      :ok ->
        :telemetry.execute(
          [:eve_dmv, :event_bus, :message_broadcast],
          %{count: 1},
          %{topic: topic}
        )
  ...
  defp extract_event_id(event) do
    Map.get(event, :id) ||
      Map.get(event, :killmail_id) ||
      Map.get(event, :character_id) ||
      Map.get(event, :battle_id)
  end

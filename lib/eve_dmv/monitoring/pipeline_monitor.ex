defmodule EveDmv.Monitoring.PipelineMonitor do
  ...
  defp analyze_health(state) do
    ...
    success_rate = calculate_success_rate(metrics.messages_processed, metrics.messages_failed)
    issues = if success_rate < 95.0 do
    ...
    issues = if metrics.last_failure && 
    ...
    issues = if metrics.last_success && 
    ...
    issues = Enum.reduce(metrics.errors_by_type, issues, fn {error_type, count}, acc ->
  ...
  @doc false
  def handle_pipeline_telemetry([:eve_dmv, :killmail, :processed], _measurements, _metadata, _config) do
    record_message_processed(0) # Processing time tracked separately
  end
  ...
  def handle_pipeline_telemetry([:eve_dmv, :killmail, :batch_size], measurements, _metadata, _config) do

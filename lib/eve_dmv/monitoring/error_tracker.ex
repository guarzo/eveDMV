defmodule EveDmv.Monitoring.ErrorTracker do
  ...
  @error_ttl :timer.hours(24) # Keep errors for 24 hours
  ...
  def handle_call({:get_error_stats, error_code}, _from, state) do
    stats = case :ets.lookup(:error_stats_ets, error_code) do
  ...
  def handle_call({:get_recent_errors, minutes}, _from, state) do
    ...
    errors = :ets.tab2list(@table_name)
    ...
    case :ets.lookup(:error_stats_ets, error_code) do
      [{^error_code, stats}] ->
        updated_stats = %{stats |
          count: stats.count + 1,
          last_seen: now,
          retry_count: if(ErrorCodes.retryable?(error_code), do: stats.retry_count + 1, else: stats.retry_count)
        }
        :ets.insert(:error_stats_ets, {error_code, updated_stats})
  ...
  defp calculate_retry_success_rate do
    stats = :ets.tab2list(:error_stats_ets)
    |> Enum.map(fn {_code, stats} -> stats end)
  ...
  defp cleanup_old_errors do
    ...
    old_records = :ets.tab2list(@table_name)

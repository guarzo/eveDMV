defmodule EveDmv.Core.Utils.TimeUtils do
  @moduledoc """
  ...
  @doc """
  Calculate minutes since a past DateTime.

  ## Examples

      iex> minutes_since(DateTimeUtils.add(DateTime.utc_now(), -1800, :second))
    30
  """
  def minutes_since(past_datetime) when is_struct(past_datetime, DateTime) do
    div(time_since(past_datetime), 60)
  end

  @doc """
  Calculate seconds since a past DateTime.

  Alias for time_since/1 for compatibility.

  ## Examples

      iex> seconds_since(DateTimeUtils.add(DateTime.utc_now(), -3600, :second))
    3600
  """
  def seconds_since(past_datetime) when is_struct(past_datetime, DateTime) do
    time_since(past_datetime)
  end
  ...
  ## Examples
      ...
      iex> hours_since(DateTimeUtils.add(DateTime.utc_now(), -7200, :second))
    2
  """
  def hours_since(past_datetime) when is_struct(past_datetime, DateTime) do
    div(time_since(past_datetime), 3600)
  end
  ...
  ## Examples
      ...
      iex> format_relative_time(DateTimeUtils.add(DateTime.utc_now(), -3600, :second))
      "1 hour ago"
  """
  def format_relative_time(datetime) when is_struct(datetime, DateTime) do
    seconds_ago = time_since(datetime)
  ...
  @doc """
  Parse a datetime string into a DateTime struct.

  Supports ISO 8601 format and common datetime patterns.

  ## Examples

      iex> parse_datetime("2024-07-01T12:00:00Z")
      {:ok, ~U[2024-07-01 12:00:00Z]}
  """

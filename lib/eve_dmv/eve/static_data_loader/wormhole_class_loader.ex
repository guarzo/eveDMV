  ...
  defp update_systems_batch(systems, wormhole_data) do
    ...
    |> Enum.map(fn system ->
      ...
      if wormhole_class_id do
        Logger.debug(
          "Updating system #{system.system_id} with wormhole class #{wormhole_class_id}"
        )

        case Ash.update(system, %{wormhole_class_id: wormhole_class_id},
               action: :update_wormhole_data,
               domain: Api
             ) do
          {:ok, _updated_system} ->

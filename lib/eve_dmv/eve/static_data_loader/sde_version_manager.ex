defmodule EveDmv.Eve.StaticDataLoader.SdeVersionManager do
  ...
  defp version_needs_update?(current, latest) do
    case {current, latest} do
      {nil, _} -> true  # No current version, always update
  ...
  defp update_sde_data(new_version) do
    ...
    case results do
      %{wormhole_classes: {:ok, classes_count}, wormhole_effects: {:ok, effects_count}} ->
        ...
        Logger.info("Updated #{classes_count} wormhole classes and #{effects_count} wormhole effects")
    ...
    case SolarSystem
      ...
      {:ok, systems} ->
        ...
        |> Enum.each(fn system ->
          ...
          }, action: :update_sde_version, domain: Api)
        end)

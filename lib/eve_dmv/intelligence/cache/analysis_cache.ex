defmodule EveDmv.Intelligence.Cache.AnalysisCache do
  ...
  def get_vetting_analysis(character_id) do
    ...
    get_or_compute(cache_key, Cache.vetting_analysis_ttl(), fn ->
      # Use Intelligence Engine for threat analysis (vetting)
      case EveDmv.IntelligenceEngine.analyze(:threat, character_id, scope: :full) do
        {:ok, _result} = success -> success
        error -> error
      end
    end)
  ...
  def get_threat_analysis(character_id) do
    ...
    get_or_compute(cache_key, Cache.threat_analysis_ttl(), fn ->
      ...
      case EveDmv.IntelligenceEngine.analyze(:threat, character_id, scope: :basic) do
        {:ok, _result} = success -> success
        error -> error
      end
    end)
  ...
  def get_member_activity_analysis(character_id) do
    ...
    get_or_compute(cache_key, Cache.member_activity_ttl(), fn ->
      ...
      case EveDmv.Intelligence.Analyzers.MemberActivityAnalyzer.analyze_member_activity(
             character_id,
             period_start,
             period_end
           ) do
  ...
  def get_home_defense_analysis(system_id) do
    ...
    get_or_compute(cache_key, Cache.home_defense_ttl(), fn ->
      ...
      # Use Intelligence Engine for fleet analysis (home defense)
      case EveDmv.IntelligenceEngine.analyze(:fleet, system_id, scope: :basic) do
        {:ok, _result} = success -> success
        error -> error
      end
    end)
  ...
  def get_bulk_character_analyses(character_ids) do
    ...
    missing_results =
      ...
      |> Task.async_stream(
        fn id ->
          case EveDmv.Intelligence.Analyzers.CharacterAnalyzer.analyze_character(id) do
            {:ok, result} ->
              # Cache the result
  ...
  def init(_opts) do
    ...
    cache_opts = [
      ...
      hooks: [
        Cachex.Spec.hook(
          module: EveDmv.Intelligence.Cache.AnalysisCache.TelemetryHook,
          args: []
        )
      ]
    ]
...
defmodule EveDmv.Intelligence.Cache.AnalysisCache.TelemetryHook do
  @moduledoc """

defmodule EveDmv.Intelligence.PerformanceOptimizer do
  ...
  def optimize_killmail_queries(character_id, date_range \\ nil) do
    ...
    participant_query = 
      ...
      |> Ash.Query.limit(10000)  # Prevent runaway queries
    ...
    case Ash.read(participant_query, domain: Api) do
      {:ok, participants} ->
        ...
        Logger.debug("Killmail query for character #{character_id} took #{query_time}ms, returned #{length(participants)} records")
      ...
      {:error, reason} ->
        ...
        Logger.error("Optimized killmail query failed for character #{character_id} after #{query_time}ms: #{inspect(reason)}")
        {:error, reason}
    end
  end
  ...
  def analyze_performance_metrics do
    ...
    performance_report = %{
      ...
      recommendations: generate_performance_recommendations(cache_stats, query_performance, memory_usage)
    }
    
    Logger.info("Performance analysis complete: hit_ratio=#{cache_stats.hit_ratio}%, memory=#{memory_usage.total_mb}MB")
  ...
  defp generate_performance_recommendations(cache_stats, query_performance, memory_usage) do
    ...
    recommendations = 
      if query_performance.avg_query_time_ms > 1000 do
        ["Optimize database queries - average time #{query_performance.avg_query_time_ms}ms" | recommendations]
      else
        recommendations
      end

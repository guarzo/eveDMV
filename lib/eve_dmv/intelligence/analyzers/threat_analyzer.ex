defmodule EveDmv.Intelligence.Analyzers.ThreatAnalyzer do
  @moduledoc """
  ...
  def analyze(character_id, opts \\ %{}) do
    corporation_id = Map.get(opts, :corporation_id)
    alliance_id = Map.get(opts, :alliance_id)

    cache_ttl = Config.get_cache_ttl(:threat)

    CacheHelper.get_or_compute(:threat, character_id, cache_ttl, fn ->
      do_analyze_pilot(character_id, corporation_id, alliance_id)
    end)
  end

  @impl true
  def invalidate_cache(character_id) do
    CacheHelper.invalidate_analysis(:threat, character_id)
  end
  ...
  def analyze_pilot(character_id, corporation_id \\ nil, alliance_id \\ nil) do
    # Legacy interface - delegate to behavior implementation
    opts = %{}
    opts = if corporation_id, do: Map.put(opts, :corporation_id, corporation_id), else: opts
    opts = if alliance_id, do: Map.put(opts, :alliance_id, alliance_id), else: opts
    ...
    case analyze_with_telemetry(character_id, opts) do
      ...
      {:error, reason} ->
        Logger.error("Threat analysis failed for character #{character_id}: #{inspect(reason)}")

        %{
          threat_level: :unknown,
          threat_score: 0,
          bait_probability: 0,
          analysis_reason: "Analysis failed"
        }
    end
  end
  ...
  defp do_analyze_pilot(character_id, corporation_id, alliance_id) do
    with {:ok, character_stats} <-
           TimeoutHelper.with_default_timeout(fn -> get_character_stats(character_id) end, :query),
         {:ok, recent_activity} <-
           TimeoutHelper.with_default_timeout(fn -> get_recent_activity(character_id) end, :query),
         {:ok, associates} <-
           TimeoutHelper.with_default_timeout(
             fn -> find_known_associates(character_id) end,
             :query
           ) do
      ...
      {:error, reason} ->
        {:error, "Failed to gather threat analysis data: #{inspect(reason)}"}
    end
  end
  ...
  def analyze_pilots(pilot_list) do
    batch_limit = Config.get_batch_limit(:character_analysis)
    timeout_ms = Config.get_timeout(:batch)

    if length(pilot_list) > batch_limit do
      Logger.warning(
        "Batch size #{length(pilot_list)} exceeds limit #{batch_limit}, processing in chunks"
      )
    end

    tasks =
      Enum.map(pilot_list, fn {character_id, corp_id, alliance_id} ->
        {fn ->
           case analyze_pilot(character_id, corp_id, alliance_id) do
             result when is_map(result) -> {character_id, {:ok, result}}
             error -> {character_id, {:error, error}}
           end
         end, :threat_analysis}
      end)

    results = TimeoutHelper.with_timeout_batch(tasks, timeout_ms)

    # Convert results to map, filtering out errors
    results
    |> Enum.filter(fn
      {_id, {:ok, _result}} ->
        true

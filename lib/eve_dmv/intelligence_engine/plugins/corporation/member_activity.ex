defmodule EveDmv.IntelligenceEngine.Plugins.Corporation.MemberActivity do
  @moduledoc """
  Corporation member activity analysis plugin.
  ...
  @impl true
  def analyze(corporation_id, base_data, opts) when is_integer(corporation_id) do
    start_time = System.monotonic_time()

    try do
      with {:ok, corporation_data} <- get_corporation_data(base_data, corporation_id),
           {:ok, member_stats} <- get_member_statistics(base_data, corporation_id) do
        activity_analysis = %{
        ...
        log_plugin_execution(corporation_id, duration_ms, {:ok, activity_analysis})
        ...
        {:error, _reason} = error ->
          ...
          log_plugin_execution(corporation_id, duration_ms, error)
          error
      end
    rescue
      exception ->
        handle_plugin_exception(exception, corporation_id)
    end
  end

  # Batch analysis support
  @impl true
  def analyze(corporation_ids, base_data, opts) when is_list(corporation_ids) do
    if supports_batch?() do
      corporation_ids
      |> Enum.map(fn corp_id ->
        Task.async(fn -> {corp_id, analyze(corp_id, base_data, opts)} end)
      end)
      # Longer timeout for corp analysis
      |> Enum.map(&Task.await(&1, 45_000))
      |> merge_batch_results()
      |> then(&{:ok, &1})
    else
  ...
  def plugin_info do
    %{
      name: "Member Activity Analyzer",
      description:
        "Analyzes corporation member activity, engagement levels, and organizational health",
      version: "2.0.0",
      dependencies: [:eve_database],
      tags: [:corporation, :members, :activity, :engagement],
  ...
  def cache_strategy do
    %{
      strategy: :default,
      # 10 minutes for member activity
      ttl_seconds: 600,
      cache_key_prefix: "member_activity"
    }
  end
  ...
  defp analyze_overall_activity(corporation_data, member_stats) do
    total_members = corporation_data.member_count || 0
  ...
  defp analyze_activity_distribution(member_stats) do
    ...
    activity_by_hour =
      ...
      |> Enum.flat_map(fn member ->
        Map.get(member, :activity_by_hour, %{}) |> Map.to_list()
      end)
    ...
    activity_by_day =
      member_stats
      |> Enum.flat_map(fn member ->
        Map.get(member, :activity_by_day, %{}) |> Map.to_list()
      end)
      ...
      |> Enum.map(fn {timezone, members} ->
        {timezone,
         %{
           ...
           active_count: Enum.count(members, &is_member_active?/1),
  ...
  defp calculate_participation_metrics(member_stats) do
    ...
    fleet_participants =
      Enum.count(member_stats, fn member ->
        (member.group_activity_ratio || 0.0) > 0.3
      end)

    # Corporate activity participation
    corp_activity_participants =
      Enum.count(member_stats, fn member ->
        member.corp_activity_score && member.corp_activity_score > 50
      end)
  ...
  defp analyze_retention_patterns(corporation_data) do
    member_history = corporation_data.member_history || []
    recent_joins = corporation_data.recent_joins || []
    recent_departures = corporation_data.recent_departures || []
  ...
  defp analyze_leadership_activity(corporation_data, member_stats) do
  ...
  defp get_corporation_data(base_data, corporation_id) do
    case get_in(base_data, [:corporation_data, corporation_id]) do
      nil -> {:error, :corporation_not_found}
      corp_data -> {:ok, corp_data}
    end
  end

  defp get_member_statistics(base_data, corporation_id) do
    case get_in(base_data, [:member_statistics, corporation_id]) do
      # Return empty list if no member stats
      nil -> {:ok, []}
      member_stats -> {:ok, member_stats}
    end
  end

  defp count_active_members(member_stats) do
    Enum.count(member_stats, &is_member_active?/1)
  end

  defp is_member_active?(member) do
    # Consider a member active if they have recent activity
    recent_activity = (member.recent_kills || 0) + (member.recent_losses || 0)
    last_active_days = days_since_last_active(member.last_active)
  ...
  defp days_since_last_active(last_active) do
  ...
  defp calculate_recent_activity(member_stats) do
    total_recent_activity =
      member_stats
      |> Enum.map(fn member ->
        (member.recent_kills || 0) + (member.recent_losses || 0)
      end)
      |> Enum.sum()
  ...
  defp calculate_member_activity_score(member) do
    ...
    recent_kills = member.recent_kills || 0
    recent_losses = member.recent_losses || 0
    days_since_active = days_since_last_active(member.last_active)
  ...
  defp calculate_overall_engagement_score(member_stats) do
    if length(member_stats) == 0, do: 0.0
  ...
  defp calculate_timezone_coverage_score(members) do
    active_members = Enum.count(members, &is_member_active?/1)

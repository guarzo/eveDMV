defmodule EveDmv.Contexts.Intelligence.Core.UnpredictabilityEngine do
  ...
  def analyze(character_id) do
    with {:ok, behavioral} <- BehavioralPatternAnalyzer.analyze_behavior(character_id),
      ...
      analysis = %{
        ...
        opsec_rating: assess_operational_security(behavioral, location_patterns, timing_patterns),
    ...
    case KillmailRepository.get_by_character(character_id, start_date: start_date, limit: 1000) do
      {:ok, killmails} ->
        location_analysis = %{
          ...
          system_distribution: analyze_system_distribution(killmails),
    ...
    case KillmailRepository.get_by_character(character_id, start_date: start_date, limit: 1000) do
      {:ok, killmails} ->
        timing_analysis = %{
          ...
          variance_score: calculate_timing_variance(killmails),
  ...
  defp analyze_session_unpredictability(killmails) do
    ...
    %{
      ...
      session_length_variance: calculate_variance(session_lengths),
      avg_session_interval: calculate_average(session_intervals),
    ...
    |> Enum.reduce([], fn km, sessions ->
      case sessions do
        ...
        [current_session | rest] ->
          ...
          hours_since = DateTimeUtils.diff(km.killmail_time, last_km.killmail_time, :hour)
  ...
  defp calculate_session_intervals(sessions) do
    ...
    |> Enum.chunk_every(2, 1, :discard)
    |> Enum.map(fn [s1, s2] ->
      start1 = List.first(s1).killmail_time
      start2 = List.first(s2).killmail_time
      DateTimeUtils.diff(start2, start1, :hour)
    end)
  end
  ...
  defp assess_operational_security(behavioral, location_patterns, timing_patterns) do
    ...
    cond do
      ...
      security_score >= 0.4 -> :moderate
      security_score >= 0.2 -> :poor
      true -> :very_poor
    end
  end

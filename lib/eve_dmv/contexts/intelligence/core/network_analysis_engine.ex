defmodule EveDmv.Contexts.Intelligence.Core.NetworkAnalysisEngine do
  ...
  def get_key_associates(character_id, limit \\ 10) do
    case analyze_network(character_id) do
      {:ok, analysis} ->
        associates = analysis.direct_network
  ...
  defp process_killmail_for_network(killmail, character_id, network) when is_map(killmail) and is_map(network) do
    # Extract all participants
    participants = extract_participants(killmail)
    
    # Filter to relevant participants (exclude NPC, self)
    relevant_participants = participants
      |> Enum.filter(fn p -> 
    ...
    # Add nodes
    updated_nodes = Enum.reduce(relevant_participants, network.nodes, fn pid, nodes ->
    ...
    updated_edges = Enum.reduce(relevant_participants, network.edges, fn pid, edges ->
      edge_key = create_edge_key(character_id, pid)
      
      edge_data = Map.get(edges, edge_key, %{
  ...
  defp analyze_direct_connections(network_data) do
    ...
    |> Enum.map(fn edge ->
      %{
        ...
        time_span_days: DateTime.diff(edge.last_seen, edge.first_seen, :day),
  ...
  defp calculate_interaction_strength(edge) do
    ...
    days_since_last = DateTime.diff(DateTime.utc_now(), edge.last_seen, :day)
    recency_factor = :math.exp(-days_since_last / 30.0)  # Exponential decay over 30 days
    
    # Consistency factor (regular interactions = stronger)
    time_span = max(1, DateTime.diff(edge.last_seen, edge.first_seen, :day))
    consistency_factor = interaction_count / time_span
    
    # Value factor (higher value engagements = stronger)
    total_value = edge.interactions
      |> Enum.map(&(&1.ship_value))
      |> Enum.sum()
    value_factor = :math.log10(max(1, total_value / 1_000_000))  # Log scale of millions
    
    # Combine factors
    strength = interaction_count * recency_factor * (1 + consistency_factor) * (1 + value_factor / 10)
  ...
  defp classify_relationship(edge) do
    ...
    avg_fleet_size = Enum.sum(fleet_sizes) / length(fleet_sizes)
  ...
  defp calculate_average_fleet_size(interactions) when is_list(interactions) do
    if Enum.empty?(interactions) do
      ...
      Float.round(total / length(interactions), 1)
    end
  end
  ...
  defp calculate_average_degree(degree_distribution) do
    if map_size(degree_distribution) == 0 do
      0.0
    else
      total_degree = degree_distribution |> Map.values() |> Enum.sum()
      Float.round(total_degree / map_size(degree_distribution), 2)
    end
  end
  ...
  defp calculate_clustering_coefficient(network_data) do
    ...
    if length(nodes) < 3 do
      ...
      |> Enum.filter(&(&1 > 0))  # Only consider nodes with non-zero coefficient
      
      # Global clustering coefficient is the average
      if Enum.empty?(node_coefficients) do
        0.0
      else
        avg = Enum.sum(node_coefficients) / length(node_coefficients)
        Float.round(avg, 3)
      end
    end
  end
  ...
  defp calculate_degree_centrality(character_id, network_data) do
    ...
    connections = network_data.edges
      |> Map.values()
  ...
  defp calculate_eigenvector_centrality(character_id, network_data) do
    ...
    if MapSet.size(node_connections) == 0 do
      ...
      neighbor_scores = node_connections
        |> MapSet.to_list()
  ...
  defp analyze_fleet_compositions(_), do: %{
    average_fleet_size: 0,
    common_compositions: [],
    preferred_roles: [],
    coordination_level: :none
  ...
  defp calculate_average_fleet_size_from_fleets(fleets) when is_list(fleets) and length(fleets) > 0 do
    total_size = fleets 
  ...
  defp classify_single_ship(ship_type_id) when is_integer(ship_type_id) do
    ...
    cond do
      ShipTypes.is_ship_class?(ship_type_id, :capital) or ShipTypes.is_ship_class?(ship_type_id, :supercapital) -> :capital
  ...
  defp build_period_network(_), do: %{
    node_count: 0,
    edge_count: 0,
    period_start: nil,
    period_end: nil
  ...
  defp identify_network_brokers(network_data) do
    ...
    high_degree_nodes = network_data.edges
      |> Map.values()
      |> Enum.flat_map(&(&1.participants))
      |> Enum.frequencies()

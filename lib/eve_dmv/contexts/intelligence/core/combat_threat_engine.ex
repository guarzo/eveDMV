defmodule EveDmv.Contexts.Intelligence.Core.CombatThreatEngine do
  ...
  defp count_high_value_kills(killmails, character_id) do
    ...
    |> Enum.filter(fn km ->
      ...
      km.total_value > 1_000_000_000  # 1 billion ISK
    end)
    |> length()
  end
  ...
  defp detect_capital_usage(killmails, character_id) do
    capital_ship_ids = [
      # Carriers
      23757, 23911, 23915, 24483,
      # Dreadnoughts  
      19720, 19722, 19724, 19726,
      # Supercarriers
      23913, 23917, 23919, 3514,
      # Titans
      671, 3764, 11567, 23773
    ]
  ...
  defp calculate_kill_frequency(killmails, character_id) do
    ...
    if length(kills) >= 2 do
      ...
      intervals = sorted_kills
      |> Enum.chunk_every(2, 1, :discard)
      |> Enum.map(fn [km1, km2] ->
        DateTime.diff(km2.killmail_time, km1.killmail_time, :hour)
      end)
  ...
  defp calculate_threat_score(combat_stats, recent_activity) do
    ...
    activity_score = case recent_activity.activity_level do
      :hyperactive -> 15
      :very_active -> 12
      :active -> 8
      :moderate -> 4

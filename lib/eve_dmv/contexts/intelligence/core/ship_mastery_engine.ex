defmodule EveDmv.Contexts.Intelligence.Core.ShipMasteryEngine do
  ...
  defp calculate_mastery_score(ship_prefs, ship_stats) do
    # Base score from favorite ships performance
    ship_scores = ship_prefs.favorite_ships
    |> Enum.take(5)
    |> Enum.map(fn ship ->
    ...
    avg_ship_score = if Enum.empty?(ship_scores) do
      0.0
    else
    ...
    progression_bonus = case ship_prefs.ship_progression.current_stage do
      :capital_pilot -> 0.2
      :advanced_pilot -> 0.15
  ...
  defp calculate_advanced_usage(ship_prefs) do
    ...
    advanced_percentage = Map.get(tech_dist, :tech2, 0) + 

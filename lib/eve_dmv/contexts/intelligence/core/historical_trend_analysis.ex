defmodule EveDmv.Contexts.Intelligence.Core.HistoricalTrendAnalysis do
  ...
  defp detect_recent_ship_changes(ship_usage) do
    ...
    recent_cutoff = week_key(DateTimeUtils.add(DateTime.utc_now(), -14 * 24 * 3600, :second))
  ...
  defp project_future_score(current_score, trends) do
    ...
    momentum_factor =
      case trends.threat_score_trend[:momentum] do
        :accelerating -> 1.2
        :steady -> 1.0
        :decelerating -> 0.8
        _ -> 1.0
    ...
    # 4 weeks projection
    projected = current_score + change_rate * 4 * momentum_factor
  ...
  defp calculate_threat_score_for_period(killmails, _character_id) do
    ...
    # 100M ISK = 1 point
    value_score = min(30, total_value / 100_000_000)

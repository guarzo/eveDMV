defmodule EveDmv.Contexts.Intelligence.Core.LouvainCommunityDetection do
  ...
  def detect_communities(network_data) do
    ...
    {optimized_partition, modularity} = optimize_modularity(
      initial_partition,
      adjacency,
      total_weight,
      0
  ...
  defp calculate_total_weight(adjacency) do
    ...
    |> Enum.sum()
    |> Kernel./(2.0)  # Each edge counted twice
  end
    ...
    if iteration >= 100 do
      ...
      {new_partition, improved} = 
        ...
        |> Enum.reduce({partition, false}, fn node, {current_partition, any_improved} ->
          {best_community, best_gain} = find_best_community(
            node,
            current_partition,
            adjacency,
            total_weight
  ...
  defp calculate_modularity_gain(node, from_community, to_community, partition, adjacency, total_weight, node_degree) do
    ...
    gain_to = (weight_to_community - to_degree * node_degree / (2.0 * total_weight)) / total_weight
    gain_from = (weight_from_community - (from_degree - node_degree) * node_degree / (2.0 * total_weight)) / total_weight
      ...
      |> Enum.flat_map(fn {node, neighbors} ->
        ...
        |> Enum.map(fn {neighbor, weight} ->
          ...
          if node_community != neighbor_community do
            edge_key = if node_community < neighbor_community,
  ...
  defp calculate_final_cohesion(members, adjacency) do
    if length(members) < 2 or is_nil(adjacency) do
      ...
      internal_edges = 
        ...
        |> length()
        |> Kernel./(2)  # Each edge counted twice

defmodule EveDmv.Contexts.Intelligence.Core.CharacterAnalyzer do
  ...
  def get_character_stats(character_id) do
    case IntelligenceCache.get({:character_stats, character_id}) do
      nil ->
  ...
  def handle_call({:analyze_character, character_id, opts}, from, state) do
    ...
    case IntelligenceCache.get(cache_key) do
      nil ->
  ...
  def handle_call({:analyze_batch, character_ids, opts}, _from, state) do
    ...
    end, max_concurrency: 10, timeout: @analysis_timeout)
    ...
    case Map.get(state.active_analyses, ref) do
      {from, cache_key} ->
        ...
        if match?({:ok, _}, result) do
          {:ok, analysis} = result
          IntelligenceCache.put(cache_key, analysis, ttl: @cache_ttl)
        end
  ...
  defp gather_character_data(character_id, opts) do
    ...
    tasks = if Keyword.get(opts, :include_combat_stats, true) do
      [Task.async(fn -> CombatStatsAnalyzer.analyze_combat_stats(character_id) end) | tasks]
    else
      tasks
    end
    
    tasks = if Keyword.get(opts, :include_ship_prefs, true) do
      [Task.async(fn -> ShipPreferenceAnalyzer.analyze_ship_preferences(character_id) end) | tasks]
    else
      tasks
    end
    
    tasks = if Keyword.get(opts, :include_behavior, true) do
      [Task.async(fn -> BehavioralPatternAnalyzer.analyze_behavior(character_id) end) | tasks]
    else
      tasks
    end
    
    tasks = if Keyword.get(opts, :include_performance, true) do
  ...
  defp identify_strengths(data) do
    ...
    strengths = if combat[:kill_death_ratio] > 3 do
      ["Excellent kill/death ratio" | strengths]
    else
      strengths
    end
    
    strengths = if combat[:solo_effectiveness] > 0.7 do
      ["Strong solo pilot" | strengths]
    else
      strengths
    end
    
    strengths = if ships[:capital_usage] > 0.2 do
  ...
  defp identify_weaknesses(data) do
    ...
    weaknesses = if patterns[:predictable_timezone] do
      ["Predictable activity times" | weaknesses]
    else
      weaknesses
    end
    
    weaknesses = if patterns[:limited_ship_variety] do
  ...
  defp identify_patterns(data) do
    ...
    notable = if patterns[:consistent_gang_size] do
      ["Consistent gang composition" | notable]
    else
      notable
    end
    
    notable = if patterns[:regional_preference] do
  ...
  defp build_character_profile(character_id) do
    with {:ok, analysis} <- analyze_character(character_id),
         {:ok, threat} <- EveDmv.Contexts.Intelligence.Core.ThreatAssessmentEngine.assess_threat(character_id) do

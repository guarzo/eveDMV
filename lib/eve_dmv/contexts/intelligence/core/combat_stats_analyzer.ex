defmodule EveDmv.Contexts.Intelligence.Core.CombatStatsAnalyzer do
  ...
  defp calculate_start_date(days: days) do
    DateTime.utc_now()
    |> DateTimeUtils.add(-days * 24 * 60 * 60, :second)
  end

  defp calculate_start_date(hours: hours) do
    DateTime.utc_now()
    |> DateTimeUtils.add(-hours * 60 * 60, :second)
  end
  ...
  defp calculate_basic_stats(killmails, character_id) do
    ...
    # Calculate ratios
    kd_ratio =
      if total_losses > 0, do: total_kills / total_losses, else: Float.round(total_kills * 1.0, 2)

    isk_efficiency =
      if isk_lost > 0, do: isk_destroyed / (isk_destroyed + isk_lost) * 100, else: 100.0

    # Gang size analysis
    {solo_kills, small_gang_kills, fleet_kills} = categorize_by_gang_size(kills)
    {solo_losses, _small_gang_losses, _fleet_losses} = categorize_by_gang_size(losses)
  ...
  defp determine_weapon_from_ship(ship_info) do
    ...
    cond do
      Enum.any?(bonuses, &String.contains?(&1, "projectile")) ->
        :projectile

      Enum.any?(bonuses, &String.contains?(&1, "laser")) ->
        :laser

      Enum.any?(bonuses, &String.contains?(&1, "hybrid")) ->
        :hybrid

      Enum.any?(bonuses, &String.contains?(&1, "missile")) ->
        :missile

      Enum.any?(bonuses, &String.contains?(&1, "drone")) ->
        :drone
        ...
        km.victim.character_id != character_id and
          Enum.any?(km.attackers, fn att ->
            att.character_id == character_id and
              capital_or_blops?(att.ship_type_id)
          end)
      end)
      |> length()
    ...
    brawling_ships =
      ...
      |> Enum.filter(fn km ->
        km.victim.character_id != character_id and
          Enum.any?(km.attackers, fn att ->
            att.character_id == character_id and
              brawling_ship?(att.ship_type_id)
          end)
      end)
      |> length()
  ...
  defp capital_or_blops?(ship_type_id) do
  ...
  defp brawling_ship?(ship_type_id) do
    case ShipTypes.get_ship_attributes(ship_type_id) do
      {:ok, ship_info} ->
  ...
  defp kiting_ship?(ship_type_id) do
    case ShipTypes.get_ship_attributes(ship_type_id) do
      {:ok, ship_info} ->

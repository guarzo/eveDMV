defmodule EveDmv.Contexts.Intelligence.Core.GangEffectivenessEngine do
  ...
  defp analyze_coordination(killmails, character_id) do
    gang_activities = killmails
    ...
    if length(gang_activities) < 5 do
      ...
      coordination_scores = gang_activities
      |> Enum.map(fn km ->
        score = 0
      ...
      Float.round(avg_coordination * 5, 1)  # Scale to 0-5
    end
  end
  ...
  defp analyze_role_diversity(killmail) do
    ship_classes = killmail.attackers
  ...
  defp classify_ship_role(ship_type_id) do
    ...
    cond do
      ship_type_id < 1000 -> :tackle
      ship_type_id < 2000 -> :dps
      ship_type_id < 3000 -> :support
      ship_type_id < 4000 -> :logistics
      true -> :special
    end
  end
  ...
  defp identify_gang_role(killmails, character_id) do
    ...
    if Enum.empty?(gang_participations) do
      ...
      avg_damage = gang_participations
      |> Enum.map(& &1.damage_share)
      |> Enum.sum()
      |> Kernel./(length(gang_participations))
      
      final_blow_rate = gang_participations
  ...
  defp detect_leadership_patterns(killmails, character_id) do
    ...
    indicators = %{
      ...
      target_calling_patterns: false  # Would need more data
    }
    
    leadership_score = indicators
    |> Map.values()
    |> Enum.count(& &1)
    |> Kernel./(4)
  ...
  defp detect_early_aggression(gang_kills, character_id) do
    ...
    first_aggressor_count = gang_kills
    |> Enum.count(fn km ->

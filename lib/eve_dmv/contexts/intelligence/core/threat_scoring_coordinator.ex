defmodule EveDmv.Contexts.Intelligence.Core.ThreatScoringCoordinator do
  ...
  def get_gang_threat_assessment(character_ids) do
    with {:ok, coordination} <- coordinate_threat_analysis(character_ids) do
      gang_assessment = %{
        ...
        engagement_recommendation: generate_gang_engagement_rec(coordination.individual_assessments)
      }
      {:ok, gang_assessment}
    end
  end
  ...
  defp find_highest_threat(assessments) do
    case Enum.max_by(assessments, fn {_, assessment} -> assessment.overall_score end, fn -> nil end) do
      nil -> nil
      {char_id, assessment} ->
        %{
          character_id: char_id,
  ...
  defp find_lowest_threat(assessments) do
    case Enum.min_by(assessments, fn {_, assessment} -> assessment.overall_score end, fn -> nil end) do
      nil -> nil
      {char_id, assessment} ->
        %{
          character_id: char_id,
  ...
  defp calculate_combined_threat(assessments) do
    if map_size(assessments) == 0 do
      ...
      base_threat = Enum.sum(scores) / length(scores)  # Average
      peak_threat = Enum.max(scores)  # Highest individual
      mass_threat = :math.log10(length(scores) + 1) * 10  # Numbers advantage
  ...
  defp analyze_gang_composition(assessments) do
    if map_size(assessments) == 0 do
      ...
      threat_levels = assessments
      |> Map.values()
      |> Enum.map(& &1.threat_level)
      |> Enum.frequencies()
      
      # Analyze specializations from aspect scores
      specializations = assessments
      |> Map.values()

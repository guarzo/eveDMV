defmodule EveDmv.Contexts.Intelligence.Services.ProfileService do
  ...
  def share_profile(character_id, sharing_options \\ []) do
    with {:ok, profile} <- generate_full_profile(character_id) do
      ...
      IntelligenceCache.put({:profile_share, share_token}, share_record, ttl: :timer.hours(24 * 7))
  ...
  def get_shared_profile(share_token) do
    case IntelligenceCache.get({:profile_share, share_token}) do
      nil -> {:error, :not_found}
      share_record ->
  ...
  defp build_comprehensive_profile(character_id) do
    ...
    with {:ok, character_analysis} <- CharacterAnalyzer.analyze_character(character_id),
      ...
      profile = %{
        ...
        strategic_assessment: build_strategic_assessment(character_analysis, threat_assessment, performance_analysis),
        
        # Intelligence Summary
        intelligence_summary: build_intelligence_summary(character_analysis, threat_assessment, performance_analysis)
      }
  ...
  defp build_threat_profile(threat_assessment) do
    %{
      ...
      engagement_recommendation: generate_engagement_recommendation(threat_assessment.threat_level),
  ...
  defp build_strategic_assessment(character_analysis, threat_assessment, performance_analysis) do
    %{
      ...
      engagement_recommendations: generate_detailed_engagement_recs(threat_assessment, performance_analysis),
      ...
      recruitment_potential: assess_recruitment_potential(character_analysis, performance_analysis)
    }
  end
  ...
  defp generate_detailed_engagement_recs(threat_assessment, performance_analysis) do
    ...
    performance_modifiers = if performance_analysis.core_metrics.solo_effectiveness > 70 do
      ["Avoid solo engagements" | performance_modifiers]
    else
      performance_modifiers
    end
    
    performance_modifiers = if performance_analysis.core_metrics.versatility_score > 80 do
  ...
  defp generate_counter_strategies(character_analysis, threat_assessment) do
    ...
    strategies = if threat_assessment.aspect_scores.combat_effectiveness > 70 do
      ["Use numerical advantage", "Avoid direct confrontation" | strategies]
    else
      strategies
    end
    
    strategies = if threat_assessment.aspect_scores.operational_security > 70 do
  ...
  defp assess_intelligence_value(character_analysis, threat_assessment) do
    ...
    bonus = 0
    bonus = if character_analysis.classifications.pilot_type == :elite_solo_hunter, do: bonus + 0.2, else: bonus
  ...
  defp assess_recruitment_potential(character_analysis, performance_analysis) do
    ...
    skill_score = case character_analysis.classifications.skill_level do
      :elite -> 1.0
      :veteran -> 0.8
      :experienced -> 0.6
      :intermediate -> 0.4
  ...
  defp extract_key_insights(character_analysis, threat_assessment) do
    ...
    insights = if threat_assessment.overall_score > 80 do
    ...
    Enum.take(insights, 5)  # Top 5 insights
  end
  ...
  defp summarize_threat_level(threat_assessment) do
    %{
      ...
      primary_aspects: threat_assessment.aspect_scores
  ...
  defp generate_tactical_notes(character_analysis) do
    ...
    notes = case character_analysis.classifications.pilot_type do
    ...
    notes = case character_analysis.classifications.engagement_style do
  ...
  defp assess_data_confidence(character_analysis) do
    ...
    total_activities = character_analysis.summary.total_kills + character_analysis.summary.total_losses
  ...
  defp export_as_yaml(profile) do
    case YamlElixir.write_to_string(profile) do
      {:ok, yaml} -> {:ok, yaml}
  ...
  defp calculate_expiry(options) do
    case Keyword.get(options, :expiry) do
      %DateTime{} = expiry -> expiry
      days when is_integer(days) -> DateTime.utc_now() |> DateTime.add(days * 24 * 60 * 60, :second)
      _ -> DateTime.utc_now() |> DateTime.add(7 * 24 * 60 * 60, :second)  # Default 7 days
    end
  end

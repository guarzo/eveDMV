defmodule EveDmv.Contexts.Intelligence.Services.ComparisonService do
  ...
  defp should_include_aspect?(aspect, :all), do: true
  ...
  defp generate_comparison_summary(character_data, aspects) do
    ...
    summary = if Map.has_key?(List.first(Map.values(character_data)), :threat) do
      threat_scores = character_data
      |> Map.values()
  ...
  defp generate_aspect_rankings(character_data, aspects) do
    ...
    # Threat rankings
    rankings = if should_include_aspect?(:threat, aspects) do
      threat_ranking = character_data
    ...
    # Combat rankings
    rankings = if should_include_aspect?(:combat, aspects) do
      combat_ranking = character_data
  ...
  defp generate_comparison_insights(character_data, _aspects) do
    ...
    insights = case find_highest_threat(character_data) do
      nil -> insights
    ...
    # Find outliers
    insights = case find_performance_outliers(character_data) do
      [] -> insights
  ...
  defp perform_character_ranking(character_ids, metric) do
    ...
    character_metrics = character_ids
    |> Task.async_stream(fn char_id ->

defmodule EveDmv.Contexts.Combat.Core.ParticipantAnalyzer do
  ...
  defp merge_participant_instances(_character_id, instances) do
    ...
    |> Enum.reduce(base, fn instance, acc ->
      ...
      |> Map.update(:ships_used, [instance.ship_type_id], &Enum.uniq([instance.ship_type_id | &1]))
      |> Map.update(:total_damage_done, instance[:damage_done] || 0, &(&1 + (instance[:damage_done] || 0)))
      |> Map.update(:total_damage_taken, instance[:damage_taken] || 0, &(&1 + (instance[:damage_taken] || 0)))
  ...
  defp calculate_aggregate_metrics(participants) do
    %{
      ...
      unique_corporations: participants |> Enum.map(& &1.corporation_id) |> Enum.uniq() |> length(),
      unique_alliances: participants |> Enum.map(& &1.alliance_id) |> Enum.reject(&is_nil/1) |> Enum.uniq() |> length(),
  ...
  defp add_kill_death_counts(participant) do
    kills = Enum.count(participant[:killmail_ids] || [], fn _ ->
  ...
  defp add_threat_assessment(participant) do
    threat_level = case participant[:combat_score] do
  ...
  defp calculate_activity_score(participant) do
    ...
    if time_span > 0 do
      appearances / time_span * 60 # Activities per hour
    else
      0
    end
  end
  ...
  defp estimate_ship_value(_ship_type_ids) do
    # TODO: Implement actual ship value estimation
    50
  end
  ...
  defp get_ships_used(killmails, character_id) do
    killmails
    |> Enum.flat_map(fn km ->
      victim_ship = if get_in(km.victim, ["character_id"]) == character_id do
        [%{

defmodule EveDmv.Contexts.Combat.Core.FleetCompositionAnalyzer do
  ...
  def analyze_composition(killmails) when is_list(killmails) do
    ...
    compositions = Enum.map(sides, fn {side, side_ships} ->
  ...
  defp group_ships_by_side(ships, killmails) do
    ...
    affiliation_map = participants.all_participants
  ...
  defp calculate_composition_breakdown(ship_classes) do
    ...
    %{
      dps_percentage: calculate_class_percentage(ship_classes, [:battleship, :battlecruiser, :cruiser], total),
  ...
  defp calculate_mobility_rating(ship_classes) do
    ...
    cruiser_weight = (ship_classes[:cruiser][:count] || 0) * 1
    
    total_ships = ship_classes
    |> Map.values()
    |> Enum.reduce(0, & &1.count + &2)
  ...
  defp assess_fleet_capabilities(ship_classes) do
    ...
    capabilities = if (ship_classes[:capital][:count] || 0) > 0 do
    ...
    subcap_count = [:battleship, :battlecruiser, :cruiser]
    |> Enum.reduce(0, fn class, acc ->
      acc + (ship_classes[class][:count] || 0)
    end)
    
    capabilities = if subcap_count > 10 do
    ...
    capabilities = if tackle_count > 5 do
  ...
  defp identify_weaknesses(ship_classes) do
    ...
    weaknesses = if estimate_logistics_percentage(ship_classes) < 10 do
    ...
    tackle_count = (ship_classes[:frigate][:count] || 0) + (ship_classes[:destroyer][:count] || 0)
    weaknesses = if tackle_count < 3 do
      [:insufficient_tackle | weaknesses]
    else
      weaknesses
    end
    ...
    if total_ships > 0 do
      max_class_percentage = ship_classes
      |> Map.values()
      |> Enum.map(& &1.percentage)
      |> Enum.max()
      
      weaknesses = if max_class_percentage > 60 do
        [:unbalanced_composition | weaknesses]
      else
        weaknesses
      end
    end
  ...
  defp check_armor_doctrine(composition) do
    ...
    armor_ships = composition.ship_classes
  ...
  defp check_shield_doctrine(composition) do
  ...
  defp calculate_side_effectiveness(composition) do
    ...
    overall = (scores.composition_score + scores.capability_score + 
  ...
  defp generate_recommendations(composition, scores) do
    ...
    recommendations = if scores.composition_score < 50 do
    ...
    recommendations = recommendations ++ Enum.map(composition.weaknesses, fn weakness ->
      case weakness do

defmodule EveDmv.Contexts.Combat.Core.ParticipantAnalyzer.AffiliationAnalyzer do
  ...
  defp add_damage_pattern_edges(edges, participants) do
    ...
    damage_edges = participants
    |> analyze_damage_patterns()
    |> Enum.map(fn {char1, char2, relationship} ->
      weight = case relationship do
        :friendly -> 0.7  # They don't shoot each other
        :hostile -> -0.9  # They actively shoot each other
      end
  ...
  defp add_temporal_edges(edges, participants) do
    ...
    temporal_edges = participants
    |> Enum.combination(2)
    |> Enum.filter(fn [p1, p2] ->
      similar_appearance_times?(p1, p2)
    end)
    |> Enum.map(fn [p1, p2] ->
  ...
  defp identify_third_parties(sides) do
    ...
    total_members = Enum.reduce(sides, 0, fn side, acc ->
  ...
  defp calculate_affiliation_strength(participant, side, sides) do
    # How strongly affiliated is this participant with their side?
    side_data = Enum.find(sides, fn s ->

defmodule EveDmv.Contexts.Combat.Core.ParticipantAnalyzer.RoleClassifier do
  ...
  defp determine_secondary_roles(participant, primary_role) do
    ...
    roles = if is_high_damage_dealer?(participant) && primary_role != :damage_dealer do
      [:damage_dealer | roles]
    else
      roles
    end
    
    roles = if has_tackle_behavior?(participant) && primary_role != :tackle do
      [:tackle | roles]
    else
      roles
    end
    
    roles = if has_command_behavior?(participant) && primary_role != :fleet_commander do
      [:squad_leader | roles]
    else
      roles
    end
  ...
  defp is_fleet_commander?(participant) do
    ...
    avg_damage < 5000  # Low damage per kill suggests non-DPS role
  end

defmodule EveDmv.Contexts.Combat.Core.ParticipantAnalyzer.ExperienceAnalyzer do
  ...
  defp calculate_tactical_awareness(participant) do
    ...
    final_blow_ratio = if participant[:kills] > 0 do
  ...
  defp identify_experience_indicators(participant) do
    ...
    # Positive indicators
    indicators = if participant[:kills] > participant[:deaths] * 2 do
      ["positive_kd_ratio" | indicators]
    else
      indicators
    end
    
    indicators = if participant[:survival_time] == :survived && participant[:appearances] > 5 do
      ["good_survival_instincts" | indicators]
    else
      indicators
    end
    
    indicators = if participant[:efficiency_rating] > 70 do
      ["high_damage_efficiency" | indicators]
    else
      indicators
    end
    
    indicators = if length(participant[:ships_used] || []) > 2 do
    ...
    # Negative indicators
    indicators = if participant[:deaths] > participant[:kills] * 2 do

defmodule EveDmv.Contexts.Combat.Services.ZkillboardImporter do
  ...
  alias EveDmv.Contexts.Combat.Core.{BattleDetector, BattleAnalyzer}
  ...
  defp transform_zkillboard_kill(zkb_kill) do
    try do
      %KillmailRaw{
        ...
        moon_id: zkb_kill["moon_id"],
        war_id: zkb_kill["war_id"],
        victim: zkb_kill["victim"],
        attackers: zkb_kill["attackers"] || [],
        zkb: zkb_kill["zkb"] || %{},
  ...
  defp save_or_update_killmail(killmail_data) do
    ...
    case Repo.get(KillmailRaw, killmail_data.killmail_id) do
      nil ->
        # Insert new killmail
        case Repo.insert(killmail_data) do
          {:ok, killmail} -> killmail
          {:error, _} -> nil
        end
        
      existing ->
        # Update existing killmail with zkillboard data
        case Repo.update(Ecto.Changeset.change(existing, Map.from_struct(killmail_data))) do
          {:ok, killmail} -> killmail
          {:error, _} -> existing
        end
    end
  ...
  defp calculate_zkb_stats(killmails) do
    %{
      total_value: Enum.reduce(killmails, 0, fn km, acc ->
        acc + get_in(km.zkb, ["totalValue"]) || 0
      end),
      points: Enum.reduce(killmails, 0, fn km, acc ->
        acc + get_in(km.zkb, ["points"]) || 0
      end),
      npc_kills: Enum.count(killmails, fn km ->
        get_in(km.zkb, ["npc"]) == true
      end),
      solo_kills: Enum.count(killmails, fn km ->
        get_in(km.zkb, ["solo"]) == true
      end),
      awox_kills: Enum.count(killmails, fn km ->
        get_in(km.zkb, ["awox"]) == true
      end)
    }
  end
  
  defp enrich_battle_data(battle, zkb_data) do
    %{
  ...
  defp extract_battle_tags(zkb_data) do
    ...
    tags = if zkb_data.zkb_stats.npc_kills > 0 do
      ["has_npc_kills" | tags]
    else
      tags
    end
    
    tags = if zkb_data.zkb_stats.solo_kills > 0 do
      ["has_solo_kills" | tags]
    else
      tags
    end
    
    tags = if zkb_data.zkb_stats.awox_kills > 0 do
      ["has_awox" | tags]
    else
      tags
    end
    
    tags = if zkb_data.zkb_stats.total_value > 10_000_000_000 do
  ...
  defp process_historical_kills(kills) do
    ...
    processed = Enum.map(kills, &transform_zkillboard_kill/1)
    |> Enum.reject(&is_nil/1)
    ...
    case BattleDetector.detect_battles(saved) do
      {:ok, battles} ->
        ...
        created_battles = Enum.map(battles, fn battle_data ->

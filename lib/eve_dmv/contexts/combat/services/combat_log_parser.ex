defmodule EveDmv.Contexts.Combat.Services.CombatLogParser do
  ...
  alias EveDmv.Contexts.Combat.Core.{BattleDetector, TimelineBuilder}
  ...
  def convert_to_killmail_format(combat_log_id) do
    with {:ok, log} <- get_combat_log(combat_log_id),
      ...
      killmails = Enum.map(destruction_events, fn destruction ->
  ...
  defp add_session_context(events) do
    ...
    |> Enum.reduce({[], nil}, fn event, {acc, current_session} ->
      case event.type do
        ...
        _ ->
          event_with_session = if current_session do
  ...
  defp detect_battles_from_log(parsed_data) do
    ...
    pseudo_killmails = parsed_data.damage_events
    ...
    case BattleDetector.detect_battles(pseudo_killmails) do
      {:ok, battles} ->
        ...
        created = Enum.map(battles, fn battle_data ->
  ...
  defp group_by_destruction(damage_events) do
    ...
    |> Enum.filter(& &1.total_damage > 1000) # Filter out minor damage
  end
  ...
  defp create_pseudo_killmail(destruction_group) do
    ...
    %{
      ...
      attackers: Enum.map(destruction_group.sources, fn source ->
        %{
          "character_name" => source,
          "damage_done" => calculate_source_damage(source, destruction_group.events)
        }
      end),
      solar_system_id: 30000142, # Unknown system
      zkb: %{"totalValue" => 0}
    }
  end
  ...
  defp calculate_total_damage_received(damage_events) do
  ...
  defp calculate_average_dps(damage_events) do
    return 0 if Enum.empty?(damage_events)
  ...
  defp build_killmail_from_destruction(destruction, all_events) do
    attackers = destruction.damage_events
    ...
    %{
      ...
      attackers: attackers,
      solar_system_id: 30000142, # Unknown
      source: :combat_log
    }
  end
end
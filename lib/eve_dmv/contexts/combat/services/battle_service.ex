defmodule EveDmv.Contexts.Combat.Services.BattleService do
  @moduledoc """
  ...
  alias EveDmv.Contexts.Combat.Resources.{Battle, BattleKillmail, CombatLog}
  alias EveDmv.Contexts.Combat.Core.BattleDetector
  alias EveDmv.Repo
  import Ecto.Query
  ...
  def get_battle(battle_id) do
    case Ash.get(Battle, battle_id) do
      {:ok, battle} -> {:ok, battle}
  ...
  def split_battle(battle_id, split_time) do
    with {:ok, battle} <- get_battle(battle_id),
         {:ok, killmails} <- get_battle_killmails(battle_id),
         {before_kms, after_kms} <- split_killmails_by_time(killmails, split_time) do
      
      # Create two new battles
  ...
  defp apply_filters(query, filters) do
    Enum.reduce(filters, query, fn
      {:system_id, system_id}, q ->
        Ash.Query.filter(q, system_id == ^system_id)
        
      {:start_time, start_time}, q ->
        Ash.Query.filter(q, start_time >= ^start_time)
        
      {:end_time, end_time}, q ->
        Ash.Query.filter(q, end_time <= ^end_time)
        
      {:min_participants, min}, q ->
        Ash.Query.filter(q, participant_count >= ^min)
        
      {:min_value, min_value}, q ->
        Ash.Query.filter(q, total_value >= ^min_value)
  ...
  defp link_killmails_to_battle(battle_id, killmail_ids) do
    ...
    battle_killmails = Enum.map(killmail_ids, fn km_id ->
      %{
        battle_id: battle_id,
        killmail_id: km_id,
  ...
  defp apply_search_filter({:participant_name, name}, query) do
    # Would need to join with participants table
    query
  end
  
  defp apply_search_filter({:corporation_id, corp_id}, query) do
    # Would need to join with participants
    query
  end
  
  defp apply_search_filter({:alliance_id, alliance_id}, query) do
    # Would need to join with participants
    query
  end
  ...
  defp fetch_battles(battle_ids) do
    battles = Battle
    |> Ash.Query.filter(id in ^battle_ids)
    |> Ash.read!()
  ...
  defp move_battle_killmails(from_battle_id, to_battle_id) do
    BattleKillmail
    |> Ash.Query.filter(battle_id == ^from_battle_id)
    |> Ash.bulk_update(:update, %{battle_id: to_battle_id})
  end
  ...
  defp split_killmails_by_time(killmails, split_time) do
    {before, after_split} = Enum.split_with(killmails, fn km ->

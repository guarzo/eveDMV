defmodule EveDmv.Contexts.Corporation.Core.SecurityAnalyzer do
  ...
  defp perform_security_analysis(corporation_id) do
    ...
    with {:ok, members} <- CorporationRepository.get_corporation_members(corporation_id),
         {:ok, activity_analysis} <- MemberActivityAnalyzer.analyze_member_activity(corporation_id),
         {:ok, risk_assessment} <- MemberRiskAssessment.assess_member_risks(corporation_id),
         {:ok, security_risks} <- analyze_security_risks(members, activity_analysis, risk_assessment) do
  ...
  defp analyze_security_risks(members, activity_analysis, risk_assessment) do
    ...
    security_risks = %{
      ...
      overall_risk_level: determine_overall_security_risk(infiltration_risks, awox_risks, spy_indicators)
    }
  ...
  defp analyze_infiltration_risk_factors(members, activity_analysis) do
    ...
    new_members = Enum.filter(members, fn member ->
    ...
    low_activity_members = Enum.filter(member_metrics, fn metric ->
    ...
    %{
      ...
      risk_level: assess_infiltration_risk_level(new_members, low_activity_members, suspicious_members),
      high_risk_members: compile_high_risk_members(new_members, low_activity_members, suspicious_members)
    }
  end
  ...
  defp assess_infiltration_risk_level(new_members, low_activity_members, suspicious_members) do
    ...
    new_member_ratio = length(new_members) / 100  # Normalized
  ...
  defp compile_high_risk_members(new_members, low_activity_members, _suspicious_members) do
    ...
    new_member_risks = Enum.map(new_members, fn member ->
      %{
    ...
    low_activity_risks = Enum.map(low_activity_members, fn metric ->
      %{
  ...
  defp analyze_awox_risks(_members, risk_assessment) do
    ...
    potential_awoxers = flight_risks
  ...
  defp calculate_awox_risk_score(member) do
    base_score = member.risk_score * 0.5  # Base from general risk
    
    # Additional factors for AWOX risk
    performance_penalty = if Enum.any?(member.risk_indicators, &String.contains?(&1, "Poor combat")) do
      20
    else
      0
    end
    
    new_member_penalty = if Enum.any?(member.risk_indicators, &String.contains?(&1, "new member")) do
      15
    else
      0
    end
  ...
  defp identify_potential_spies(_members, activity_analysis) do
    ...
    spy_indicators = member_metrics
    |> Enum.map(fn metric ->
      indicators = []
      
      # Low combat participation but active
      indicators = if metric.total_killmails < 5 && metric.last_seen &&
      ...
      indicators = if metric.gang_participation.solo_ratio > 0.9 do
      ...
      indicators = if length(metric.peak_activity_hours) > 0 &&
  ...
  defp unusual_timezone_pattern?(peak_hours) do
    ...
    length(peak_hours) == 1 || # Only active in one hour
    (length(peak_hours) > 0 && Enum.all?(peak_hours, &(&1 in [0, 6, 12, 18]))) # Regular interval checks
  end
  ...
  defp assess_operational_security(members, activity_analysis) do
    ...
    %{
      ...
      overall_opsec_score: calculate_opsec_score(predictability_score, opsec_violations, fleet_security)
    }
  end
  ...
  defp assess_activity_predictability(activity_analysis) do
    ...
    cond do
      length(peak_hours) <= 2 ->
        :highly_predictable  # Active only in narrow window
      length(peak_hours) <= 4 ->
  ...
  defp calculate_opsec_score(predictability, violations, fleet_security) do
    base_score = 70  # Start with decent score
    
    # Deduct for predictability
    predictability_penalty = case predictability do
  ...
  defp analyze_member_vetting_status(members) do
    ...
    well_vetted = Enum.count(members, fn member ->
      member.join_date && DateTime.diff(DateTime.utc_now(), member.join_date, :day) > 90
    end)
    
    recently_vetted = Enum.count(members, fn member ->
      member.join_date && 
      DateTime.diff(DateTime.utc_now(), member.join_date, :day) <= 90 &&
      DateTime.diff(DateTime.utc_now(), member.join_date, :day) > 30
    end)
    
    unvetted = Enum.count(members, fn member ->
      is_nil(member.join_date) || DateTime.diff(DateTime.utc_now(), member.join_date, :day) <= 30
    end)
  ...
  defp identify_suspicious_patterns(_members, activity_analysis) do
    ...
    suspicious_patterns = if lurkers > 3 do
  ...
  defp identify_security_vulnerabilities(security_risks) do
    ...
    vulnerabilities = if security_risks.infiltration_risks.risk_level in [:high, :critical] do
    ...
    vulnerabilities = if security_risks.awox_risk_assessment.awox_risk_level in [:high, :critical] do
    ...
    # Spy vulnerabilities
    vulnerabilities = if security_risks.spy_indicators.spy_risk_level in [:high, :critical] do
  ...
  defp extract_infiltration_indicators(infiltration_risks) do
    ...
    indicators = if infiltration_risks.new_member_ratio > 20 do
      ["High ratio of new members (#{infiltration_risks.new_member_ratio}%)" | indicators]
    else
      indicators
    end
    
    indicators = if infiltration_risks.low_activity_count > 5 do
  ...
  defp extract_behavioral_indicators(activity_analysis) do
    ...
    indicators = if Map.get(summary, :participation_rate, 100) < 40 do
  ...
  defp extract_operational_indicators(security_risks) do
    ...
    indicators = if length(security_risks.spy_indicators.potential_spies) > 0 do
  ...
  defp calculate_security_score(security_risks) do
    ...
    infiltration_penalty = case security_risks.infiltration_risks.risk_level do
      :critical -> 30
      :high -> 20
      :moderate -> 10
      :low -> 5
    ...
    awox_penalty = case security_risks.awox_risk_assessment.awox_risk_level do
      :critical -> 25
      :high -> 15
      :moderate -> 10
      :low -> 5
    ...
    spy_penalty = case security_risks.spy_indicators.spy_risk_level do
      :high -> 20
      :moderate -> 10
      :low -> 5
      _ -> 0
  ...
  defp generate_security_recommendations(security_risks) do
    ...
    recommendations = case security_risks.infiltration_risks.risk_level do
    ...
    recommendations = case security_risks.awox_risk_assessment.awox_risk_level do
      level when level in [:high, :critical] ->
    ...
    # Spy recommendations
    recommendations = case security_risks.spy_indicators.spy_risk_level do
      :high ->
    ...
    recommendations = if security_risks.overall_risk_level in [:high, :critical] do
  ...
  defp get_character_killmail_history(character_id) do
    start_date = DateTime.utc_now() |> DateTime.add(-180, :day)  # 6 months
  ...
  defp identify_security_flags(killmail_data, activity_data, _affiliation_data) do
    ...
    flags = if killmail_data.kills == 0 && killmail_data.losses > 10 do
      ["Only losses recorded - potential feeder" | flags]
    else
      flags
    end
    
    flags = if killmail_data.friendly_fire_incidents > 0 do
    ...
    flags = if activity_data.consistency_score < 30 do
  ...
  defp calculate_consistency_from_killmails(killmails) do
    if length(killmails) < 10 do
      ...
      days_with_activity = killmails
      |> Enum.map(fn km -> DateTime.to_date(km.killmail_time) end)
      |> Enum.uniq()
      |> length()
      
      total_days = DateTime.diff(
  ...
  defp extract_affiliations_from_killmails(killmails, character_id) do
    ...
    character_corporations = killmails
    |> Enum.filter(fn km -> 
    ...
    current_corporation = case List.last(character_corporations) do
      {_time, corp_id} -> corp_id
      nil -> nil
    end
    
    # Track corporation changes
    corporation_changes = character_corporations
    |> Enum.chunk_every(2, 1, :discard)
  ...
  defp extract_known_associates_from_killmails(character_id, killmails) do
    ...
    |> Enum.flat_map(fn km ->
      if km.victim.character_id == character_id do
        ...
        other_attackers = Enum.reject(km.attackers, fn att -> att.character_id == character_id end)
    ...
    |> Enum.frequencies()
    |> Enum.filter(fn {_char_id, count} -> count >= 3 end)  # Appeared together 3+ times

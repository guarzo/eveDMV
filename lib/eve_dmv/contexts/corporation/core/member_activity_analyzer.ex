defmodule EveDmv.Contexts.Corporation.Core.MemberActivityAnalyzer do
  ...
  defp collect_member_activity(member, start_date) do
    with {:ok, killmails} <-
           CharacterRepository.get_character_killmails_since(member.character_id, start_date),
      ...
      activity = %{
        ...
        last_seen: member.last_seen,
        recent_kills:
          length(Enum.filter(killmails, &(&1.victim.character_id != member.character_id))),
        recent_losses:
          length(Enum.filter(killmails, &(&1.victim.character_id == member.character_id))),
  ...
  defp calculate_activity_score(killmails, member, character_stats) do
    # Base activity from killmail count
    base_score = length(killmails) * 2
    ...
    # Max 10 points for tenure
    tenure_bonus = min(tenure_days / 365 * 5, 10)
    ...
    performance_bonus =
      if character_stats && character_stats.kill_death_ratio do
  ...
  defp calculate_contribution_rating(killmails, character_stats) do
    ...
    # Max 5 points from activity
    activity_rating = min(length(killmails) / 10, 5)

    # Performance rating
    performance_rating =
      if character_stats && character_stats.kill_death_ratio do
        # Max 5 points from K/D
        min(Decimal.to_float(character_stats.kill_death_ratio), 5)
      else
        # Average assumption
        2.5
      end
  ...
  defp calculate_consistency_score(killmails, _start_date) do
  ...
  defp build_activity_summary(activity_data, participation_data) do
    ...
    avg_activity_score =
      if map_size(activity_data) > 0 do
        activity_data
        |> Map.values()

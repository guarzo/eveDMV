defmodule EveDmv.Contexts.FleetOperations.Analyzers.PilotAnalyzer do
  ...
  """
  def get_available_pilots(corporation_id, base_data \\ %{}, opts \\ []) when is_integer(corporation_id) do
    try do
      ...
      exception -> Result.error(:pilots_query_failed, "Failed to get available pilots: #{inspect(exception)}")
    end
  end
  ...
  """
  def optimize_pilot_assignments(doctrine_template, available_pilots, base_data \\ %{}, opts \\ []) do
    try do
      assignments = %{}
        ...
        |> Enum.reduce(assignments, fn {role, role_data}, acc ->
          ...
          Enum.reduce(assigned_pilots, acc, fn pilot, acc2 ->
            assignment = %{
              ...
              assigned_role: role,
              assigned_ship: select_best_ship_for_pilot(pilot, Map.get(role_data, "preferred_ships", [])),
      ...
      exception -> Result.error(:assignment_optimization_failed, "Pilot assignment optimization error: #{inspect(exception)}")
    end
  end
  ...
  def calculate_readiness_metrics(pilot_assignments, base_data \\ %{}, opts \\ []) do
    try do
      ...
      exception -> Result.error(:readiness_calculation_failed, "Readiness metrics calculation error: #{inspect(exception)}")
    end
  end
  ...
  defp calculate_availability_metrics(pilot_data) do
    ...
    availability_level = determine_availability_level(activity_score, recently_active, fleet_ready)
  ...
  defp is_recently_active?(last_activity_date) do
    case last_activity_date do
      nil -> true  # Assume active if no data
      last_date ->
  ...
  defp determine_backup_roles(role_suitability) do
    ...
    |> Enum.drop(1)  # Skip the primary role
    |> Enum.take(2)  # Take top 2 backup roles

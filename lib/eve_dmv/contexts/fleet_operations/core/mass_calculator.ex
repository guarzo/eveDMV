defmodule EveDmv.Contexts.FleetOperations.Core.MassCalculator do
  ...
  @wormhole_mass_limits %{
    # C1-C3 connections
    "X877" => 1_000_000_000,  # 1B kg
    "B274" => 2_000_000_000,  # 2B kg  
    "A982" => 3_000_000_000,  # 3B kg
    
    # C4-C5 connections  
    "O477" => 1_800_000_000,  # 1.8B kg
    "L477" => 2_500_000_000,  # 2.5B kg
    "M555" => 1_000_000_000,  # 1B kg
    
    # C6 connections
    "A239" => 1_350_000_000,  # 1.35B kg
    "U574" => 2_250_000_000,  # 2.25B kg
    
    # K-space connections
    "K162" => 3_000_000_000,  # 3B kg (varies by source)
    "B449" => 2_000_000_000   # 2B kg
  }
  ...
  defp calculate_efficiency_score(mass_data, role) do
    ...
    case role do
      :tackle ->
        ...
        cond do
          base_mass < 1_500_000 -> 1.0      # Interceptors (~1.2-1.3M kg)
          base_mass < 2_000_000 -> 0.9      # T2 Frigates (~1.5-1.8M kg)
          base_mass < 3_000_000 -> 0.8      # T1 Frigates (~1.8-2.5M kg)
          base_mass < 15_000_000 -> 0.6     # Destroyers (~2-8M kg)
          base_mass < 50_000_000 -> 0.4     # Light cruisers
          true -> 0.2
        end
      ...
      :dps ->
        ...
        cond do
          base_mass < 15_000_000 -> 0.8     # Light cruisers
          base_mass < 20_000_000 -> 0.95    # Heavy cruisers (~12-18M kg)
          base_mass < 80_000_000 -> 1.0     # Battlecruisers (~60-70M kg)
          base_mass < 120_000_000 -> 1.0    # Battleships (~100-110M kg)
          base_mass < 1_500_000_000 -> 0.7  # Capitals (~1.2-1.4B kg)
          true -> 0.4                       # Supercapitals
        end
      
      :logistics ->
        # Logistics: moderate mass preferred
        cond do
          base_mass < 20_000_000 -> 1.0     # Cruiser logi
          base_mass < 200_000_000 -> 0.8    # Battleship logi
          true -> 0.5
        end
      ...
      :support ->
        ...
        cond do
          base_mass < 5_000_000 -> 0.9      # Small support
          base_mass < 50_000_000 -> 1.0     # Medium support
          true -> 0.7
        end
  ...
  defp estimate_mass_by_ship_type(_), do: 50_000_000  # Default for unknown types
  
  defp estimate_mass_by_ship_class(size_class) when is_binary(size_class) do
    case size_class do
      "frigate" -> 1_200_000       # ~1.2M kg typical frigate
      "destroyer" -> 1_800_000     # ~1.8M kg typical destroyer  
      "cruiser" -> 12_000_000      # ~12M kg typical cruiser
      "battlecruiser" -> 60_000_000 # ~60M kg typical battlecruiser
      "battleship" -> 100_000_000   # ~100M kg typical battleship
      "capital" -> 1_200_000_000    # ~1.2B kg typical capital
      "supercapital" -> 2_500_000_000 # ~2.5B kg typical supercapital
      _ -> 50_000_000               # Default estimate
    end
  end
  ...
  defp estimate_mass_by_ship_class_fallback(ship_type_id) when is_integer(ship_type_id) do
    ...
    cond do
      ship_type_id < 1000 -> 1_500_000      # Small ships (frigates)
      ship_type_id < 5000 -> 12_000_000     # Medium ships (cruisers)
      ship_type_id < 10000 -> 60_000_000    # Large ships (battlecruisers)
      ship_type_id < 20000 -> 100_000_000   # Battleships
      true -> 300_000_000                   # Capitals and specials
    end
  end
  
  defp optimize_heavy_assault_fleet(mass_limit) do
    # Heavy assault optimization focuses on maximum DPS within mass limit
    target_mass = trunc(mass_limit * 0.85)  # Use 85% of limit for safety
  ...
  defp optimize_fast_attack_fleet(mass_limit) do
    ...
    target_mass = trunc(mass_limit * 0.6)  # Use less mass for faster movement
  ...
  defp estimate_fleet_dps(composition) do
    ...
    Enum.reduce(composition, 0, fn {ship_class, count}, acc ->
      dps_per_ship = case ship_class do
        :battleships -> 800
        :battlecruisers -> 600
        :cruisers -> 400
        :destroyers -> 250
  ...
  def calculate_jump_mass_sequence(ships, wormhole_chain) when is_list(ships) do
    ...
    case wormhole_chain do
      ...
      wormholes when is_list(wormholes) ->
        ...
        results = Enum.map(wormholes, fn wh ->

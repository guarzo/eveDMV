defmodule EveDmv.Contexts.WormholeOperations.Domain.HomeDefenseAnalyzer do
  ...
  def analyze_defense_capabilities(corporation_id) do
    ...
    try do
      ...
      defense_readiness = calculate_defense_readiness(active_members, available_ships, member_activity)
      ...
      error ->
        Logger.error("Failed to analyze defense capabilities for corporation #{corporation_id}: #{inspect(error)}")
  ...
  def assess_system_vulnerabilities(system_id) do
    ...
    try do
      ...
      vulnerability_score = calculate_vulnerability_score(entry_points, blind_spots, recent_activity)
      ...
      error ->
        Logger.error("Failed to assess vulnerabilities for system #{system_id}: #{inspect(error)}")
  ...
  def calculate_defense_readiness_score(corporation_id) do
    ...
    try do
      ...
      error ->
        Logger.error("Failed to calculate defense readiness score for corporation #{corporation_id}: #{inspect(error)}")
  ...
  def analyze_system_defense(system_id) do
    ...
    try do
      ...
      vulnerabilities = identify_system_vulnerabilities(vulnerability_assessment, defense_analyses)
      ...
      defense_recommendations = generate_system_defense_recommendations(vulnerabilities, defensive_assets)
      ...
      error ->
        Logger.error("Failed to analyze system defense for system #{system_id}: #{inspect(error)}")
        {:error, :analysis_failed}
    end
  end
  ...
  def generate_defense_recommendations(corporation_id) do
    ...
    try do
      ...
      error ->
        Logger.error("Failed to generate defense recommendations for corporation #{corporation_id}: #{inspect(error)}")
  ...
  defp calculate_recent_activity_factor(member_activity) do
    ...
    recent_activity = Enum.count(member_activity, fn activity ->
  ...
  defp estimate_response_time(member_activity, timezone_coverage) do
    ...
    base_response = 30 # minutes
    
    # Adjust based on timezone coverage
    coverage_factor = case timezone_coverage.coverage_quality do
      :excellent -> 0.7
      :good -> 0.8
      :fair -> 1.0
      :poor -> 1.3
  ...
  defp calculate_fleet_strength(member_ships) do
    ...
    frigate_strength = Map.get(ship_categories, :frigate, 0) * 1
  ...
  defp generate_doctrine_recommendations(type_frequencies) do
    ...
    recommendations = if total_ships < 20 do
    ...
    ship_type_count = Map.size(type_frequencies)
    recommendations = if ship_type_count > 10 do
  ...
  defp identify_blind_spots(topology, entry_points) do
    # Identify areas with poor coverage
    total_entries = length(entry_points)
    monitored_entries = Enum.count(entry_points, fn entry -> 
      entry.monitoring_status == :monitored
    end)
    
    blind_spot_percentage = if total_entries > 0 do
    ...
    unmonitored_entries = Enum.filter(entry_points, fn entry ->
      entry.monitoring_status != :monitored
    end)
  ...
  defp calculate_coverage_gap(entry) do
    ...
    threat_multiplier = case entry.threat_level do
      :high -> 1.5
      :medium -> 1.0
      :low -> 0.7
    end
  ...
  defp generate_coverage_recommendations(blind_spots, entry_points) do
    ...
    recommendations = if length(blind_spots) > 2 do
    ...
    high_threat_entries = Enum.count(entry_points, fn entry ->
      entry.threat_level == :high
    end)
    
    recommendations = if high_threat_entries > 1 do
  ...
  defp generate_escape_routes(connection_count, route_type) do
    ...
    route_count = case route_type do
  ...
  defp assess_defensive_positions(topology, entry_points) do
  ...
  defp generate_system_defense_recommendations(vulnerabilities, defensive_assets) do
    ...
    recommendations = if length(vulnerabilities) > 3 do
    ...
    recommendations = if defensive_assets.active_members < 10 do
      ["Increase active member participation" | recommendations]
    else
      recommendations
    end
    
    recommendations = if defensive_assets.available_ships < 20 do
  ...
  defp generate_timezone_recommendations(timezone_coverage) do
    ...
    recommendations = if coverage_pct < 50 do
      [%{
    ...
    recommendations = if tz_categories.dominant_tz != :mixed do
      [%{
  ...
  defp generate_fleet_recommendations(defensive_doctrines) do
    ...
    recommendations = if coherence < 0.5 do
      [%{
  ...
  defp generate_response_recommendations(response_time_estimate) do
    ...
    recommendations = if response_time_estimate > 45 do
      [%{
        type: :response_time,
        priority: :high,
  ...
  defp generate_activity_recommendations(active_members) do
    ...
    recommendations = if active_members < 15 do
      [%{
  ...
  defp generate_tactical_recommendations(defense_analysis, threat_event) do
    ...
    recommendations = case fleet_strength do
      :minimal -> 
        [%{
    ...
    recommendations = if response_time > 60 do
      [%{
        type: :rapid_response,
        priority: :high,
  ...
  defp generate_strategic_recommendations(defense_analysis, threat_type) do
    ...
    recommendations = if tz_coverage < 60 do
      [%{
    ...
    recommendations = case threat_type do
      :capital_escalation ->
        [%{
  ...
  defp generate_resource_recommendations(defense_analysis, threat_event) do
    ...
    recommendations = if available_ships < 30 do
      [%{
    ...
    recommendations = if threat_value > 1_000_000_000 do
      [%{
  ...
  defp generate_communication_recommendations(threat_severity, defense_analysis) do
    ...
    recommendations = case threat_severity do
      :critical ->
        [%{
    ...
    recommendations = if active_members > 50 do
      [%{

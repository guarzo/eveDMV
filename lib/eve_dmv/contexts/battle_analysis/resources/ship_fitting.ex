defmodule EveDmv.Contexts.BattleAnalysis.Resources.ShipFitting do
  ...
  attributes do
    ...
    attribute(:source, :atom,
      constraints: [one_of: [:eft, :pyfa, :ingame, :manual, :detected]],
      default: :manual
    )

    # Fitting data
    # Original format
    attribute(:raw_fitting, :string, allow_nil?: false)
    # Structured format
    attribute(:parsed_fitting, :map, default: %{})
        ...
        case parse_eft_fitting(eft_text) do
          ...
          {:error, reason} ->
            Ash.Changeset.add_error(changeset,
              field: :eft_text,
              message: "Invalid EFT format: #{reason}"
            )
        end
      end)
    end
      ...
      change(fn changeset, _ ->
        ...
        case parse_pyfa_fitting(pyfa_xml) do
          ...
          {:error, reason} ->
            Ash.Changeset.add_error(changeset,
              field: :pyfa_xml,
              message: "Invalid PyFA format: #{reason}"
            )
        end
      end)
    end
  ...
  defp parse_eft_modules(lines) do
    {_current_section, modules} =
      Enum.reduce(lines, {:unknown, %{high: [], mid: [], low: [], rigs: [], cargo: []}}, fn line,
                                                                                            {section,
                                                                                             mods} ->
        cond do
          String.trim(line) == "" ->
  ...
  defp detect_slot_type(module_name) do
    ...
    cond do
      ...
      String.contains?(module_name, ["Armor", "Damage", "Gyrostabilizer", "Heat Sink", "Magnetic"]) ->
        :low

      String.contains?(module_name, ["Rig", "Cache", "Accelerator"]) ->
        :rigs
  ...
  defp extract_fitting_from_killmail(killmail) do
    ...
    %{
      ...
      # HiSlot0-7
      high_slots: extract_slot_items(items_by_slot, 27..34),
      # MedSlot0-7
      mid_slots: extract_slot_items(items_by_slot, 19..26),
      # LoSlot0-7
      low_slots: extract_slot_items(items_by_slot, 11..18),
      # RigSlot0-2
      rig_slots: extract_slot_items(items_by_slot, 92..94),
  ...
  defp estimate_ehp_from_modules(parsed_fitting, ship_type_id) do
    ...
    tank_bonus =
      Enum.count(tank_modules, fn mod ->
  ...
  defp estimate_speed_from_modules(parsed_fitting, ship_type_id) do
    ...
    base_speed =
      cond do
        # Frigates
        ship_type_id in 582..650 -> 350
        # Cruisers
        ship_type_id in 620..634 -> 200
        # Battleships
        ship_type_id in 638..645 -> 100
        true -> 150
      end
  ...
  defp estimate_sig_from_modules(parsed_fitting, ship_type_id) do
    ...
    base_sig =
      cond do
        # Frigates
        ship_type_id in 582..650 -> 40
        # Cruisers
        ship_type_id in 620..634 -> 150
        # Battleships
        ship_type_id in 638..645 -> 400
        true -> 100
      end

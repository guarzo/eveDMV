defmodule EveDmv.Contexts.BattleAnalysis.Domain.BattleTimelineService do
  ...
  def analyze_battle_sequence(battles) when is_list(battles) do
    sorted_battles = Enum.sort_by(battles, fn b -> 
  ...
  defp identify_battle_phases(events) do
    ...
    # Escalation phases
    escalation_phases = identify_escalations(events, initial_phase)
    phases = phases ++ escalation_phases
    
    # Cleanup/final phase
    final_phase = identify_final_phase(events)
    phases = phases ++ [final_phase]
      ...
      [first | _] ->
        ...
        initial_events = 
          ...
          |> Enum.take_while(fn event ->
            ...
            time_diff <= 120  # Within 2 minutes of first kill
          end)
  ...
  defp analyze_kill_rate_change(events) do
    ...
    kill_rate = if duration_seconds > 0 do
    ...
    %{
      ...
      is_escalation: kill_rate > 2.0  # More than 2 kills per minute
    }
  end
  
  defp identify_final_phase(events) do
    case Enum.reverse(events) do
      [] -> nil
      [last | _] ->
        # Find the cleanup kills at the end
        final_events = 
          events
          |> Enum.reverse()
          |> Enum.take_while(fn event ->
            time_diff = NaiveDateTime.diff(last.timestamp, event.timestamp, :second)
            time_diff <= 300  # Within 5 minutes of last kill
          end)
          |> Enum.reverse()
        
        if length(final_events) > 1 do
          %{
            phase_type: :cleanup,
            start_time: List.first(final_events).timestamp,
            end_time: last.timestamp,
            event_count: length(final_events),
            description: "Cleanup phase with #{length(final_events)} final kills"
          }
        else
          nil
        end
  ...
  defp analyze_fleet_composition_over_time(events) do
    ...
    time_windows = chunk_events_by_time_window(events, 300)  # 5-minute windows
    ...
    Enum.map(time_windows, fn {window_start, window_events} ->
      %{
        ...
        ship_composition: analyze_ship_types_in_window(window_events),
  ...
  defp analyze_ship_types_in_window(events) do
    # Combine victim and attacker ships
    victim_ships = Enum.map(events, & &1.victim.ship_type_id)
    ...
    |> Enum.map(fn {ship_type_id, count} ->
      %{ship_type_id: ship_type_id, count: count}
    end)
  ...
  defp analyze_battle_sides(events) do
    ...
    group_interactions = 
      interactions
      |> Enum.group_by(fn i ->
        attacker_group = Map.get(corp_groups, i.attacker_corp, i.attacker_alliance || "standalone_#{i.attacker_corp}")
        victim_group = Map.get(corp_groups, i.victim_corp, i.victim_alliance || "standalone_#{i.victim_corp}")
    ...
    hostile_map = 
      ...
      |> Enum.reduce(%{}, fn interaction, acc ->
        if interaction.attacker_group != interaction.victim_group do
          acc
          |> Map.update(interaction.attacker_group, [interaction.victim_group], &[interaction.victim_group | &1])
          |> Map.update(interaction.victim_group, [interaction.attacker_group], &[interaction.attacker_group | &1])
        else
          acc
        end
      end)
  ...
  defp identify_key_moments(events, phases) do
    ...
    # First blood
    moments = if first_blood = List.first(events) do
      [%{
        type: :first_blood,
    ...
    moments = if largest_kill = Enum.max_by(events, & &1.attacker_count, fn -> nil end) do
      [%{
  ...
  defp generate_battle_summary(events, phases, key_moments) do
    ...
    %{
      ...
      description: "Battle with #{length(events)} kills across #{length(phases)} phases, featuring #{key_moment_count} key moments"
    }
  end
  ...
  defp are_battles_connected?(battle1, battle2) do
    ...
    if time_gap <= 30 do
      ...
      shared_participants = MapSet.intersection(
  ...
  defp track_participant_flow(battles) do
    ...
    |> Enum.map(fn [battle1, battle2] ->
      ...
      %{
        ...
        continuing_participants: MapSet.intersection(participants1, participants2) |> MapSet.size(),

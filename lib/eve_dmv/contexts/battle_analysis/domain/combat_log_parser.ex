defmodule EveDmv.Contexts.BattleAnalysis.Domain.CombatLogParser do
  ...
  def correlate_with_killmails(combat_events, killmails) do
    ...
    event_windows = group_events_by_time(combat_events, 60) # 1 minute windows
  ...
  defp parse_line(line) do
    cond do
      String.contains?(line, "\tCombat\t") && String.contains?(line, " from ") && Regex.match?(~r/\d+ from/, line) ->
    ...
    case Regex.run(~r/\t(\d+) from ([^\(]+)\(([^\)]+)\) - ([^-]+) - ([^\t\n\r]+)/, line) do
      [_, damage_str, attacker_name, attacker_ship, weapon, quality_str] ->
        ...
        %{
          ...
          damage: damage,
          from: from,
          to: nil,  # This log is from victim's perspective
  ...
  defp filter_by_pilot(events, pilot_name) do
    Enum.filter(events, fn event ->
      case event.type do
        ...
        _ -> true  # Include other events for context
      end
    end)
  end
  ...
  defp generate_summary(events, _pilot_name) do
    ...
    %{
      total_damage_dealt: 0,  # This log perspective doesn't show outgoing damage
      total_damage_taken: pilot_damage_taken,
      unique_targets: unique_attackers,  # Attackers shooting at us
  ...
  defp group_events_by_time(events, window_seconds) do
    ...
    |> Enum.group_by(fn event ->
      if event.timestamp do
        ...
        unix = DateTime.from_naive!(event.timestamp, "Etc/UTC") |> DateTime.to_unix()
  ...
  defp find_events_near_time(event_windows, target_time) do
    target_unix = DateTime.from_naive!(target_time, "Etc/UTC") |> DateTime.to_unix()
  ...
  defp build_damage_timeline(events, killmail) do
    damage_events = 
      events
      |> Enum.filter(& &1.type == :damage)
      |> Enum.filter(& &1.to == killmail.victim_character_name)
  ...
  defp analyze_final_blow(events, killmail) do
    ...
    %{
      ...
      final_blow_quality: final_events |> List.last() |> case do
        nil -> nil
        event -> event.quality
      end,
  ...
  defp average_damage(events) when length(events) == 0, do: 0
  ...
  defp build_damage_over_time(events) do
    ...
    |> Enum.filter(& &1.type == :damage)
    |> Enum.sort_by(& &1.timestamp)
    |> Enum.chunk_every(10) # Group by 10 events
    |> Enum.map(fn chunk ->
      %{

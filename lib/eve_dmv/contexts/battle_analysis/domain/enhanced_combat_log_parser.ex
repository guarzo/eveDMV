defmodule EveDmv.Contexts.BattleAnalysis.Domain.EnhancedCombatLogParser do
  ...
  defp parse_line(line) do
    cond do
      ...
      String.contains?(line, "\tCombat\t") && Regex.match?(~r/\d+ to [^\[]+\[[^\]]+\]\([^\)]+\) - [^-]+ - (Hits|Penetrates|Smashes|Wrecks|Glances Off|Grazes)/, line) ->
        parse_outgoing_damage_line(line)
      
      # Incoming damage: "15 from Kragden[GI.N](Covetor) - Hornet II - Glances Off"
      String.contains?(line, "\tCombat\t") && Regex.match?(~r/\d+ from [^\[]+\[[^\]]+\]\([^\)]+\) - [^-]+ - (Hits|Penetrates|Smashes|Wrecks|Glances Off|Grazes)/, line) ->
        parse_incoming_damage_line(line)
      
      # Miss patterns: "Hornet II belonging to Nitlar Nirad misses you completely"
      String.contains?(line, "\tCombat\t") && String.contains?(line, "misses") && String.contains?(line, "completely") ->
        parse_miss_line(line)
      
      # Module attempts: "Warp scramble attempt from you to"
      String.contains?(line, "\tCombat\t") && Regex.match?(~r/(Warp scramble|Warp disruption) attempt/, line) ->
      ...
      String.contains?(line, "\tCombat\t") && Regex.match?(~r/(Shield booster|Armor repair|Hull repair|Ancillary) activated/, line) ->
    ...
    cond do
      String.contains?(line, "attempt from you to") ->
        ...
        %{
          ...
          module: module_type,
          direction: :outgoing,
          success: nil  # Will be determined by follow-up events
        }
  ...
  defp parse_energy_drain_line(line) do
    ...
    case Regex.run(~r/\t-?(\d+) GJ energy drained to \[([^\]]+)\]\s*([^\-]+) - ([^\t\n\r]+)/, line) do
  ...
  defp parse_defensive_module_line(line) do
    ...
    %{
      ...
      timestamp: timestamp,
      module: module_type,
      trigger: :damage_response  # Assume triggered by damage
    }
  end
  ...
  defp parse_session_line(line) do
    ...
    action = case Regex.run(~r/Session change: (.+)$/, line) do
      [_, a] -> a
      _ -> "Unknown"
    end
  ...
  defp analyze_defensive_reactions(events) do
    ...
    %{
      ...
      damage_before_defense: calculate_damage_before_defense(defensive_activations, damage_received)
    }
  end
    ...
    target_stats = 
      ...
      |> Enum.map(fn {target, target_events} ->
        {target, %{
          ...
          avg_application: Enum.sum(Enum.map(target_events, & &1.application_percentage)) / length(target_events),
  ...
  defp filter_by_time(events, _start_time, _end_time), do: events  # Implement time filtering
  
  defp filter_by_pilot(events, nil), do: events
  defp filter_by_pilot(events, _pilot_name), do: events  # Implement pilot filtering
  ...
  defp generate_tactical_recommendations(tactical_analysis, _events) do
    ...
    # Damage application recommendations
    rec_1 = if tactical_analysis.damage_application[:average_application] && 
       tactical_analysis.damage_application.average_application < 60 do
      initial_recommendations ++ ["Consider using tracking enhancers or webs to improve hit quality (#{tactical_analysis.damage_application.average_application}% avg application)"]
    else
      initial_recommendations
    end
    
    # Range management recommendations
    rec_2 = if tactical_analysis.range_management[:range_failures] > 5 do
      rec_1 ++ ["Improve range management - #{tactical_analysis.range_management.range_failures} out-of-range attempts detected"]
    else
      rec_1
    end
    
    # Defensive recommendations
    final_rec = if tactical_analysis.defensive_reactions[:average_reaction_time] > 3 do
      rec_2 ++ ["Activate defensive modules earlier - #{Float.round(tactical_analysis.defensive_reactions.average_reaction_time, 1)}s average reaction time"]
    else
      rec_2
    end

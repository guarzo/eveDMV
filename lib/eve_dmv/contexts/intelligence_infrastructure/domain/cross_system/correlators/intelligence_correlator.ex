defmodule EveDmv.Contexts.IntelligenceInfrastructure.Domain.CrossSystem.Correlators.IntelligenceCorrelator do
  ...
  defp identify_intelligence_gaps(system_ids) do
    ...
    if length(system_ids) == 0 do
      ...
      priority_systems = 
        coverage_analysis
        |> Enum.sort_by(fn {_system, coverage} -> 
        ...
        |> Enum.map(fn {system, coverage} ->
          %{
            system_id: system,
            priority_score: Float.round(coverage.strategic_value * (1 - coverage.coverage_score), 2),
  ...
  defp assess_intelligence_quality(system_ids) do
    ...
    if length(system_ids) == 0 do
      ...
      overall_quality = (data_freshness * 0.3 + coverage_completeness * 0.4 + reliability_score * 0.3)
      ...
      %{
        ...
        recommendations: generate_quality_recommendations(overall_quality, data_freshness, coverage_completeness)
      }
    end
  end
  ...
  defp analyze_single_system_coverage(system_id) do
    ...
    query = from k in "killmails_enriched",
      ...
      select: %{
        kill_count: count(k.killmail_id),
        latest_activity: max(k.killmail_time),
        unique_entities: count(fragment("DISTINCT COALESCE(?, ?)", k.victim_character_id, k.victim_corporation_id)),
  ...
  defp calculate_data_density(metrics) do
    ...
    if metrics.kill_count == 0 do
      ...
      if time_span > 0 do
        ...
        min(1.0, density / 10) # Normalize to 0-1
      else
        0.5 # Single time point
      end
    end
  end
  ...
  defp calculate_time_overlap_score(system_times) do
    ...
    if length(system_times) < 2 do
      ...
      if length(overlaps) > 0 do
        ...
        min(1.0, avg_overlap / 24) # Normalize to 0-1 based on 24 hour overlap
      else
        0.0
      end
    end
      ...
      freshness_scores = 
        ...
        |> Enum.map(fn {_system, metrics} ->
          ...
          if day_activity > 0 do
            ...
            min(1.0, recent_ratio * 2) # Boost recent activity
          else
            0.0
          end
        end)
    ...
    if map_size(quality_metrics) == 0 do
      ...
      completeness_scores = 
        ...
        |> Enum.map(fn {_system, metrics} ->
          ...
          trend = case Map.get(metrics, :coverage_trend) do
            :improving -> 1.0
            :stable -> 0.7
            :degrading -> 0.4
            _ -> 0.5
  ...
  defp identify_missing_data_types(result) do
    ...
    missing = if Map.get(result, :kill_count, 0) == 0 do
      [:killmail_data | missing]
    else
      missing
    end
    
    missing = if Map.get(result, :unique_entities, 0) < 5 do
      [:entity_diversity | missing]
    else
      missing
    end
    
    missing = if Map.get(result, :data_types, 0) < 3 do

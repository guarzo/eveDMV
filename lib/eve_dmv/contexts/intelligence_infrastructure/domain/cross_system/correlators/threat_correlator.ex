defmodule EveDmv.Contexts.IntelligenceInfrastructure.Domain.CrossSystem.Correlators.ThreatCorrelator do
  ...
  defp fetch_threat_metrics(system_ids, start_time, end_time \\ nil) do
    ...
    query =
      from(k in "killmails_enriched",
        where:
          k.solar_system_id in ^system_ids and
    ...
    %{
      ...
      avg_kill_value:
        if(kill_count > 0, do: Map.get(result, :total_value, 0) / kill_count, else: 0),
      avg_gang_size:
        if(kill_count > 0, do: Map.get(result, :total_attackers, 0) / kill_count, else: 0),

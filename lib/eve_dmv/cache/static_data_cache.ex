defmodule EveDmv.Cache.StaticDataCache do
  ...
  def resolve_system_names(system_ids) when is_list(system_ids) do
    ...
    # Check cache first
    {cached_results, missing_ids} = Enum.reduce(unique_ids, {[], []}, fn id, {cached, missing} ->
  ...
  defp batch_fetch_systems(system_ids) do
    ...
    case Ash.read(query, domain: EveDmv.Api) do
      {:ok, systems} ->
        # Cache all results
        result_map = Map.new(systems, fn system ->
        ...
        unknown_map = Map.new(missing_ids, fn id ->
  ...
  defp batch_fetch_items(type_ids, category) do
    ...
    case Ash.read(query, domain: EveDmv.Api) do
      {:ok, items} ->
        # Cache all results
        result_map = Map.new(items, fn item ->
      ...
      {:error, error} ->
        ...
        Map.new(type_ids, fn id -> {id, "Unknown #{category |> to_string() |> String.capitalize()}"} end)
    end
  end
  ...
  defp perform_cache_warming do
    ...
    common_systems = [
      ...
      30002510,  # Rens
      30002718,  # Perimeter
      30000144,  # Jita 4-4 adjacent
      30002761,  # Ashab
      31002238,  # Common WH system from our data
    ]
    ...
    common_ships = [
      ...
      # Common PvP ships
      11993, 11987, 11985, 11989,  # Interceptors
      22442, 22444, 22446, 22448,  # Assault Frigates
      29984, 29986, 29988, 29990,  # Strategic Cruisers
    ]

defmodule EveDmvWeb.UniversalSearchLive do
  ...
  def handle_event("clear_search", _params, socket) do
    socket = 
      socket
      |> assign(query: "", results: %{systems: [], characters: [], corporations: []}, 
                show_dropdown: false, selected_index: 0, selected_category: nil)
  ...
  defp save_recent_search(type, id, query) do
    ...
    updated = [new_search | Enum.reject(recent, &(&1.id == id && &1.type == type))]
    |> Enum.take(10)
    
    AnalysisCache.put("recent_searches", updated, 86_400_000) # 24 hours
  end
  ...
  def time_ago(%DateTime{} = datetime) do
    case DateTime.diff(DateTime.utc_now(), datetime, :second) do
      ...
      seconds -> "#{div(seconds, 604800)}w ago"
    end
  end

#!/usr/bin/env python3
"""
Delegate ship class performance functions to ShipClassPerformanceAnalyzer
"""
...
# Backup the file
shutil.copy(FILE_PATH, FILE_PATH + ".backup_ship_performance")

# Read the file
with open(FILE_PATH, 'r') as f:
    content = f.read()

# Add the ShipClassPerformanceAnalyzer alias if not present
if 'ShipClassPerformanceAnalyzer' not in content:
    ...
    if match:
        alias_end = match.end()
        # Insert new alias
        new_alias = '  alias EveDmv.Contexts.CombatIntelligence.Domain.BattleAnalysis.Analyzers.ShipClassPerformanceAnalyzer\n'
...
def delegate_function(content, func_name):
    ...
    def replacer(match):
        ...
        delegation = f"{func_def}\n{indent}  ShipClassPerformanceAnalyzer.{func_name}({params})\n{end_line}"
        return delegation
    
    new_content, count = re.subn(pattern, replacer, content, flags=re.DOTALL | re.MULTILINE)
    
    if count > 0:
        print(f"Delegated {func_name} to ShipClassPerformanceAnalyzer")
    else:

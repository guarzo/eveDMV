defmodule EveDmv.Contexts.CorporationAnalysis.Analyzers.ParticipationAnalyzer do
  ...
  defp identify_peak_activity_periods(participation_data) do
    ...
    Enum.sort_by(activities_by_hour, fn {_hour, count} -> count end, :desc)
    |> Enum.take(3)
  ...
  defp identify_participation_improvement_areas(member_participations) do
    ...
    activation_areas = if inactive_ratio > 0.3, do: [:member_activation | base_areas], else: base_areas
    final_areas = if low_participation_ratio > 0.4, do: [:engagement_programs | activation_areas], else: activation_areas
  ...
  defp generate_engagement_recommendations(member_participations) do
    ...
    outreach_recommendations = if inactive_count > 0 do
      ["Implement outreach program for #{inactive_count} inactive members" | base_recommendations]
    else
      base_recommendations
    end

    final_recommendations = if low_participation_count > 2 do
      ["Create mentorship program for #{low_participation_count} low-participation members" | outreach_recommendations]
    else
  ...
  defp identify_top_performers(member_analyses) do
    Enum.sort_by(member_analyses, &calculate_member_participation_score/1, :desc)
    |> Enum.take(5)
    |> Enum.map(fn analysis ->
      %{
        character_id: analysis.character_id,

defmodule EveDmv.Contexts.FleetOperations.Analyzers.PilotAnalyzer do
  ...
  def get_available_pilots(corporation_id, base_data \\ %{}, opts \\ [])
    ...
    try do
      with {:ok, corporation_pilots} <- get_corporation_pilots(base_data, corporation_id) do
        available_pilots =
          Enum.filter(corporation_pilots, &pilot_available_for_fleet?/1)
      ...
      analysis_json ->
        case Jason.decode(analysis_json) do
          {:ok, analysis_data} ->
            ...
            Enum.map(ship_categories, fn {category, count} ->
  ...
  defp calculate_fc_suitability(pilot_data, combat_stats) do
    ...
    final_score = if combat_stats.solo_ratio < 0.5, do: efficiency_score + 0.1, else: efficiency_score
  ...
  defp calculate_logistics_suitability(pilot_data, combat_stats) do
    ...
    solo_score = if combat_stats.solo_ratio < 0.4, do: efficiency_score + 0.2, else: efficiency_score
  ...
  defp calculate_dps_suitability(pilot_data, combat_stats) do
    ...
    gang_score = if pilot_data.avg_gang_size > 2.0, do: efficiency_score + 0.2, else: efficiency_score
  ...
  defp calculate_tackle_suitability(pilot_data, combat_stats) do
    ...
    frigate_score = if Map.get(ship_groups, "Frigates", 0) > 10, do: base_score + 0.3, else: base_score
    interceptor_score = if Map.get(ship_groups, "Interceptors", 0) > 0, do: frigate_score + 0.3, else: frigate_score
    solo_score = if combat_stats.solo_ratio > 0.5, do: interceptor_score + 0.2, else: interceptor_score
  ...
  defp calculate_ewar_suitability(pilot_data, combat_stats) do
    ...
    solo_score = if combat_stats.solo_ratio < 0.3, do: efficiency_score + 0.2, else: efficiency_score
  ...
  defp determine_backup_roles(role_suitability) do
    Enum.sort_by(role_suitability, fn {_role, score} -> score end, :desc)
  ...
  defp find_backup_roles_for_pilot(pilot, doctrine_template) do
    Enum.map(doctrine_template, fn {role, _} -> role end)
  ...
  defp calculate_skill_coverage(pilot_assignments) do
    ...
    |> Enum.map(fn {role, assignments} ->
      avg_readiness =
        Enum.map(assignments, fn {_pilot_id, assignment} -> assignment.skill_readiness end)
        |> Enum.sum()
  ...
  defp calculate_availability_breakdown(pilot_assignments) do
    Enum.frequencies(Enum.map(pilot_assignments, fn {_pilot_id, assignment} -> assignment.availability end))
  end

  defp estimate_form_up_time(pilot_assignments) do
    # Estimate form-up time based on pilot availability and readiness
    avg_availability =
      Enum.map(pilot_assignments, fn {_pilot_id, assignment} ->

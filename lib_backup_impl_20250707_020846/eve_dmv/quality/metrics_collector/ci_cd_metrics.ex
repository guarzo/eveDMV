defmodule EveDmv.Quality.MetricsCollector.CiCdMetrics do
  ...
  defp analyze_github_actions do
    ...
    workflow_analysis =
      Enum.reduce(Enum.map(workflow_files, &analyze_workflow_file/1), %{ci: false, cd: false, quality: false, security: false}, fn analysis, acc ->
        %{
  ...
  defp calculate_average_script_quality(quality_checks) do
    if Enum.empty?(quality_checks) do
      ...
      total_score =
        Enum.sum(Enum.map(quality_checks, fn check ->
          score = 0

...
defmodule EveDmvWeb.IntelligenceDashboardLive do
  ...
  def mount(_params, _session, socket) do
    if connected?(socket) do
      ...
      {:ok, _ref1} = EventBus.subscribe_process(:threat_level_updated)
      {:ok, _ref2} = EventBus.subscribe_process(:battle_detected)
      {:ok, _ref3} = EventBus.subscribe_process(:intelligence_alert)

      {:ok, _ref4} =
        EventBus.subscribe_process(:system_activity_spike_detected)

      {:ok, _ref5} = EventBus.subscribe_process(:character_analysis_updated)

      # Subscribe to global intelligence updates via real-time coordinator
      {:ok, _ref6} = RealTimeCoordinator.subscribe_to_updates(:global)
  ...
  def handle_event("analyze_character", %{"character_id" => character_id_str}, socket) do
    case Integer.parse(character_id_str) do
      ...
      {_character_id, _rest} ->
        {:noreply, put_flash(socket, :error, "Invalid character ID format")}
  ...
  def handle_info({:domain_event, :battle_detected, event}, socket) do
    ...
    updated_battles =
      ...
      |> Enum.filter(fn battle ->
        ...
        DateTimeUtils.diff(DateTime.utc_now(), battle.detected_at, :second) < 7200 or
  ...
  def handle_info({:domain_event, :intelligence_alert, event}, socket) do
    ...
    updated_alerts =
      ...
      |> Enum.filter(fn alert ->
        ...
        is_nil(alert.expires_at) or DateTimeUtils.compare(DateTime.utc_now(), alert.expires_at) == :lt
      end)
      |> Enum.take(10)

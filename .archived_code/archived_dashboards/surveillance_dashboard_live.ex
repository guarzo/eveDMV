defmodule EveDmvWeb.SurveillanceDashboardLive do
  ...
  defp load_profiles(socket) do
    case ThreatSurveillance.list_profiles(%{}) do
      ...
      {:error, reason} ->
        Logger.error("Failed to load surveillance profiles: #{inspect(reason)}")
  ...
  defp get_total_alerts(time_range) do
    case safe_call(fn -> ThreatSurveillance.get_alert_metrics(time_range) end) do
  ...
  defp get_average_response_time do
    ...
    case safe_call(fn -> ThreatSurveillance.get_surveillance_metrics() end) do
  ...
  defp get_cache_hit_rate do
    ...
    case safe_call(fn -> ThreatSurveillance.get_cache_stats() end) do
    ...
    case safe_call(fn ->
      ...
      {:ok, alerts} ->
        ...
        Enum.count(alerts, fn alert ->
          DateTimeUtils.compare(alert.created_at, cutoff_time) == :gt
        end)
  ...
  defp get_profile_false_positive_rate(profile_id, _time_range) do
    ...
    case safe_call(fn -> ThreatSurveillance.get_profile(profile_id) end) do
      {:ok, profile} ->
  ...
  defp get_profile_avg_confidence(profile_id, time_range) do
    case safe_call(fn ->
           ThreatSurveillance.get_recent_alerts(profile_id: profile_id, limit: 100)
         end) do
      {:ok, alerts} ->
        cutoff_time = get_cutoff_time(time_range)
  ...
  defp get_profile_last_alert(profile_id) do
    case safe_call(fn ->
           ThreatSurveillance.get_recent_alerts(profile_id: profile_id, limit: 1)
         end) do
  ...
  defp generate_alert_trends(time_range) do
    ...
    start_time = DateTimeUtils.add(current_time, -hours * 3600, :second)
